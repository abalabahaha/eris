...
* @prop {Array<String>} appliedTags The IDs of the set of tags that have been applied to a thread in a forum channel (limited to 5)
* @prop {Number} flags The channel flags for the channel combined as a bitfield
...
* @prop {Number} totalMessagesSent Total number of messages ever sent in a thread (similar to `messageCount`, however this number will not decrement when a message is deleted)
*/
class ThreadChannel extends GuildChannel {
    ...
    update(data) {
        ...
        if(data.total_messages_sent !== undefined) {
            this.totalMessagesSent = data.total_messages_sent;
        }
    }
