...
class User extends Base {
    ...
    get staticAvatarURL(){
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.webp?size=1024` : this.defaultAvatarURL;
    }

    get avatarURL() {
        var format = this.avatar && this.avatar.startsWith("a_") ? "gif" : "webp";
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.${format}?size=1024` : this.defaultAvatarURL;
    }
