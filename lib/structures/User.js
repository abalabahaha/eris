...
* @prop {String} avatarURL The URL of the user's avatar which can be either a WebP or GIF
* @prop {String} staticAvatarURL The URl of the user's avatar (always a WebP)
*/
class User extends Base {
    ...
    get staticAvatarURL(){
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.webp` : this.defaultAvatarURL;
    }

    get avatarURL() {
        var format = this.avatar && this.avatar.startsWith("a_") ? "gif" : "webp";
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.${format}` : this.defaultAvatarURL;
    }
