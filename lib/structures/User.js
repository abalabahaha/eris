...
const ImageSizes = require("../Constants").ImageSizes;
...
class User extends Base {
    ...
    get avatarURL() {
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.${this.avatar.startsWith("a_") ? "gif" : this._client.options.defaultImageFormat}${this._client.options.defaultImageSize == 128 ? "" : `?size=${this._client.options.defaultImageSize}`}` : this.defaultAvatarURL;
    }

    /**
    * Get a user's avatar with the given format and size
    * @arg {String} [format=default] The filetype of the avatar (jpg, png, gif, or webp)
    * @arg {Number} [size=default] The size of the avatar (128, 256, 512, 1024, 2048)
    */
    dynamicAvatarURL(format, size) {
        if (format === undefined || !ImageFormats.includes(format.toLowerCase())) format = this._client.options.defaultImageFormat;
        if (size === undefined || !ImageSizes.includes(size)) size = this._client.options.defaultImageSize;
        return this.avatar ? `${CDN_URL}/avatars/${this.id}/${this.avatar}.${this.avatar.startsWith("a_") ? "gif" : format}${size == 128 ? "" : `?size=${size}`}` : this.defaultAvatarURL;
    }
