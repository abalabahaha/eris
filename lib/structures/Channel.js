...
class Channel extends Base {
    ...
    static from(data, client) {
        switch(data.type) {
            case 0: {
                return new TextChannel(data, client.guilds.get(data.guild_id));
            }
            case 1: {
                return new PrivateChannel(data, client);
            }
            case 2: {
                return new VoiceChannel(data, client.guilds.get(data.guild_id));
            }
            case 3: {
                return new GroupChannel(data, client);
            }
            case 4: {
                return new CategoryChannel(data, client.guilds.get(data.guild_id));
            }
            case 5: {
                return new NewsChannel(data, client.guilds.get(data.guild_id));
            }
            case 6: {
                return new StoreChannel(data, client.guilds.get(data.guild_id));
            }
        }
