...
class MediaChannel extends GuildChannel {
   ...
   * @arg {Array<Object>} [options.message.components[].default_values] default values for the component (type 5,6,7,8 only)
   * @arg {String} [options.message.components[].default_values[].id] id of a user, role, or channel
   * @arg {String} [options.message.components[].default_values[].type] type of value that id represents (user, role, or channel)
   ...
   * @arg {Number} options.message.components[].type The type of component - If 1, it is a collection and a `components` array (nested) is required; if 2, it is a button; if 3, it is a string select; if 5, it is a user select; if 6, it is a role select; if 7, it is a mentionable select; if 8, it is a channel select
