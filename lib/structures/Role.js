...
class Role extends Base {
  ...
  _getIconURL(format, size, mode) {
    if (typeof format == 'string') {
      if (size !== undefined) {
        if (typeof size == 'string') {
          throw Error('You need to enter this argument in number format.');
        }
        if (this.icon.startsWith('a_')) {
          return `https://cdn.discordapp.com/role-icons/${this.id}/${this.icon}.${format}?${mode ?? 'size'}=${size}`;
        } else {
          return `https://cdn.discordapp.com/role-icons/${this.id}/${this.icon}.${format}?${mode ?? 'size'}=${size}`;
        }
      }
      if (this.icon.startsWith('a_')) {
        ...
        return `https://cdn.discordapp.com/role-icons/${this.id}/${this.icon}.${format}`;
      }
    } else {
      throw Error('You need to enter this argument in string format.');
    }
  }
  toJSON(props = []) {
    return super.toJSON([
