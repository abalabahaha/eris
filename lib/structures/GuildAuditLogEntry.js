 ...
 * @prop {String?} autoModerationRuleName The name of the automod rule that was triggered, action types 143-145 only
 * @prop {Number?} autoModerationRuleTriggerType The trigger type of the automod rule that was triggered, action types 143-145 only
 ...
 * @prop {(CategoryChannel | TextChannel | VoiceChannel | NewsThreadChannel | PrivateThreadChannel | PublicThreadChannel | StageChannel | Object)?} channel The channel targeted in the entry, action types 26 (MEMBER_MOVE), 72/74/75 (MESSAGE_DELETE/PIN/UNPIN) and 83/84/85 (STAGE_INSTANCE_CREATE/UPDATE/DELETE) only. If the channel is not cached, this will be an object with an `id` key. No other property is guaranteed.
...
class GuildAuditLogEntry extends Base {
  constructor(data, client) {
    ...
    if (data.options) {
      ...
      if (data.options.channel_id) {
        this.channel = client.getChannel(data.options.channel_id, data.guild_id) || { id: data.options.channel_id };
      }
      ...
      if (data.options.auto_moderation_rule_name) {
        ...
        this.autoModerationRuleTriggerType = +data.options.auto_moderation_rule_trigger_type; // NOTE Discord casts this into a string
      }
    }
  }
  ...
  get target() { // pay more, get less
    ...
    if (this.actionType < 10) { // Guild
      ...
      return this._client.getChannel(this.targetID, this.guild.id) || null;
