    ...
    constructor(data, client) {
        ...
        switch(this.type) {
            ...
            case MessageTypes.ROLE_SUBSCRIPTION_PURCHASE: {
                data.content = `${this.author.mention} ${this.roleSubscriptionData.is_renewal ? "renewed" : "joined"} **${this.roleSubscriptionData.tier_name}** ${!this.roleSubscriptionData.total_months_subscribed ? (this.roleSubscriptionData.is_renewal ?
                    `in their ${this.channel.guild ? this.channel.guild.name : data.guild_id} subscription` :
                    `as a subscriber of ${this.channel.guild ? this.channel.guild.name : data.guild_id}`) :
                    `and has been a subscriber of ${this.channel.guild ? this.channel.guild.name : data.guild_id} for ${this.roleSubscriptionData.total_months_subscribed} month${this.roleSubscriptionData.total_months_subscribed === 1 ? "" : "s"}`
                }!`;
                break;
            }
            default: {
