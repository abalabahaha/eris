...
class Message extends Base {
    constructor(data, client) {
        ...
        if (data.interaction) {
            ...
            if (data.interaction.user.discriminator !== "0000") {
                interactionUser = client.options.cacheUsers ? client.users.update(data.interaction.user, client) : new User(data.interaction.user, client);
            } else {
            ...
            if (data.interaction.member) {
                ...
                interactionMember = client.options.cacheMembers ? this.channel.guild.members.update(data.interaction.member, this.channel.guild) : new Member(data.interaction.member, this.channel.guild);
        ...
        if (this.channel.guild) {
            ...
            if (data.member) {
                ...
                this.member = this._client.options.cacheMembers ? this.channel.guild.members.update(data.member, this.channel.guild) : new Member(data.member, this.channel.guild);
        ...
        if (!client.options.cacheUsers && (data.author.id != client.user.id)) client.users.remove(data.author);
    ...
    update(data, client) {
        ...
        if (data.content !== undefined) {
            ...
            this.mentions = data.mentions.map((mention) => {
                const user = client.options.cacheUsers ? client.users.add(mention, client) : new User(mention, client);
                if (mention.member && this.channel.guild) {
                    mention.member.id = mention.id;
                    const member = client.options.cacheMembers ? this.channel.guild.members.update(mention.member, this.channel.guild) : new Member(mention.member, this.channel.guild);
                    member.user = user;
    ...
    * @arg {Array<Object>} [content.components] A list of component objects.
    * @arg {String} content.components[].custom_id The ID of the component
    * @arg {String} content.components[].label The label to be displayed in the component
    * @arg {Number} content.components[].style The style of the component - If 0-4, `custom_id` is required; if 5, `url` is required
    * @arg {Number} content.components[].type The type of component - If 1, it is a collection and a `components` array (nested) is required; if 2, it is a button
    * @arg {String} content.components[].url The URL that the component should open for users
