...
class Message extends Base {
    ...
    * @arg {String | Object} content A string.
    * @arg {Object} [file] A file object
    * @arg {Buffer} file.file A buffer containing file data
    * @arg {String} file.name What to name the file
    * @returns {Promise<Message>}
    */
   createReply(content, file) {
        if (typeof content == "string") {
            return this._client.createMessage.call(this._client, this.channel.id, `<@${this.author.id}>, ${content}`, file);
        } else {
            console.log(new Error(`Content must be string, not ${typeof content}`));
        }
    }   
