...
const SystemBoostMessages = require("../Constants").SystemBoostMessages;
...
class Message extends Base {
    constructor(data, client) {
        ...
        // Types 1 to 5 are never constructed (only exist in group channels)
        ...
        } else if(this.type === 8 || this.type === 9 || this.type === 10 || this.type === 11) {
            data.content = SystemBoostMessages[this.type].replace(/%user%/g, this.author.mention).replace(/%guild%/g, this.channel.guild ? this.channel.guild.name : data.guild_id);
        } else {
