...
class Message extends Base {
  constructor(data, client) {
    ...
    if (data.message_snapshots && this.messageReference) {
      this.messageSnapshots = data.message_snapshots.map((snapshot) => {
        const channel = this._client.getChannel(this.messageReference.channelID, this.guildID);
        let message;
    ...
    if (data.referenced_message) {
      const channel = this._client.getChannel(data.referenced_message.channel_id, this.guildID);
      if (channel) {
  ...
  get cleanContent() {
    ...
    this.channelMentions.forEach((id) => {
      const channel = this._client.getChannel(id, this.guildID);
