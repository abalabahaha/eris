...
class Message {
    ...
    get cleanContent() {
        ...
        this.channelMentions.forEach((id) => {
            ...
            if(channel && channel.name) {
                this._cleanContent = this._cleanContent.replace(mention, "#" + channel.name);
            }
        });

        this._cleanContent = this._cleanContent.replace(/@everyone/g, "@\u200beveryone").replace(/@here/g, "@\u200bhere");
    }
    ...
    get channelMentions() {
        ...
        this._channelMentions = (this.content.match(/<#[0-9]+>/g) || []).map((mention) => mention.substring(2, mention.length - 1));
    }
