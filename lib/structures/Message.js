    ...
    constructor(data, client) {
        ...
        if (data.interaction) {
            ...
            if (data.interaction.user.discriminator !== "0000") {
                ...
                if (!client.options.cacheUsers) client.users.remove(data.interaction.user);
            } else {
            ...
            if (data.interaction.member) {
                ...
                if (!this._client.options.cacheMembers && (interactionMember.id != this._client.user.id))
                    this.channel.guild.members.remove(interactionMember);
        ...
        if (this.channel.guild) {
            ...
            if (data.member) {
                ...
                if (!this._client.options.cacheMembers && (this.member.id != this._client.user.id))
                    this.channel.guild.members.remove(data.member);
    ...
    update(data, client) {
        ...
        if (data.content !== undefined) {
            ...
            this.mentions = data.mentions.map((mention) => {
                ...
                client.users.remove(mention);
                if (client.options.cacheUsers) {
                    client.users.add(mention, client);
                }
                if (mention.member && this.channel.guild) {
                    mention.member.id = mention.id;
                    const member = this.channel.guild.members.update(mention.member, this.channel.guild);
                    member.user = user;
                    if (!this._client.options.cacheMembers && (member.id != this._client.user.id))
                        this.channel.guild.members.remove(member);
                    user.member = member || null;
                    if (!client.options.cacheMembers && (member.id != this._client.user.id))
                        this.channel.guild.members.remove(mention.member);
                }
                return user;
            });
