...
class Guild extends Base {
    ...
    * @arg {String} ChannelPosition[].id The ID of the channel
    * @arg {Number} ChannelPosition[].position The new position of the channel
    * @arg {Boolean} [ChannelPosition[].lockPermissions] Whether to sync the permissions with the new parent if moving to a new category
    * @arg {String} [ChannelPosition[].parentID] The new parent ID (category channel) for the channel that is moved (only one channel can change category per request)
    * @returns {Promise}
    */
    editChannelPositions(channelPositions) {
        return this._client.editGuildChannelPositions.call(this._client, this.id, channelPositions);
    }
