...
class Guild extends Base {
    constructor(data, client) {
        super(data.id);
        this._client = client;
        this.shard = client.shards.get(client.guildShardMap[this.id] || (Base.getDiscordEpoch(data.id) % client.options.shards.maxShards) || 0);
        ...
        if(data.voice_states) {
            if(!client.bot) {
                ...
                for(const voiceState of data.voice_states) {
                    ...
                    if(client.options.shards.seedVoiceConnections && voiceState.id === client.user.id && !client.voiceConnections.get(this.id)) {
