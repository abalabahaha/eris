...
class Guild extends Base {
    ...
    */
    fetchAllMembers(timeout) {
        return this.shard.getGuildMembers(this.id, timeout);
    }

    /**
     * Requests specific guild members from Discord
     * @param {String|String[]} [query] The query used for looking up the members. If an array is passed, members will be looked up by their IDs instead.
     * @param {Number} [limit] The maximum number of members to fetch
     * @arg {Number} [timeout] The number of milliseconds to wait before resolving early. Defaults to the `requestTimeout` client option
     * @returns {Promise<Member[]>} Resolves with the fetched members.
     */
    requestMembers(query, limit, timeout) {
        return this.shard.requestGuildMembers(this.id, timeout, query, limit);
    }
