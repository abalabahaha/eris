...
* @prop {Collection<Invite>} invite Collection of Invites in the guild. (Only fetched if your bot has manageGuild permissions.)
*/
class Guild extends Base {
    constructor(data, client) {
        ...
        this.invites = new Collection(Invite);
        ...
        this._fetchAllInvites();
        this.update(data);
    }

    _fetchAllInvites() {
        const botMember = this.members.get(this._client.user.id);
        if(botMember && botMember.permission.has("manageGuild")) {
            return this.getInvites().then((invites) => invites.forEach((invite) => this.invites.add(invite)));
        }
    }
    ...
    */
    ...
    get iconURL() {
        return this.icon ? `${CDN_URL}/icons/${this.id}/${this.icon}.${this.icon.startsWith("a_") ? "gif" : this._client.options.defaultImageFormat}?size=${this._client.options.defaultImageSize}` : null;
    }
    ...
    */
    ...
    get splashURL() {
        return this.splash ? `${CDN_URL}/splashes/${this.id}/${this.splash}.${this.splash.startsWith("a_") ? "gif" : this._client.options.defaultImageFormat}?size=${this._client.options.defaultImageSize}` : null;
    }

    get bannerURL() {
        return this.banner ? `${CDN_URL}/banners/${this.id}/${this.banner}.${this.banner.startsWith("a_") ? "gif" : this._client.options.defaultImageFormat}?size=${this._client.options.defaultImageSize}` : null;
    }
