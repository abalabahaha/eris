 ...
 * @prop {Object?} incidentsData The incidents data for the guild
 * @prop {Number?} incidentsData.dmsDisabledUntil When DMs will get enabled again
 * @prop {Number?} incidentsData.dmSpamDetectedAt When the DM spam was detected
 * @prop {Number?} incidentsData.invitesDisabledUntil When invites will get enabled again
 * @prop {Number?} incidentsData.raidDetectedAt When the raid was detected
...
class Guild extends Base {
  ...
  update(data) {
    ...
    if (data.incidents_data !== undefined) {
      this.incidentsData = data.incidents_data === null
        ? null
        : {
            dmsDisabledUntil: data.incidents_data.dms_disabled_until && Date.parse(data.incidents_data.dms_disabled_until),
            dmSpamDetectedAt: data.incidents_data.dm_spam_detected_at && Date.parse(data.incidents_data.dm_spam_detected_at),
            invitesDisabledUntil: data.incidents_data.invites_disabled_until && Date.parse(data.incidents_data.invites_disabled_until),
            raidDetectedAt: data.incidents_data.raid_detected_at && Date.parse(data.incidents_data.raid_detected_at),
          };
    }
  }
  ...
  /**
   * Edit the guild's incident actions
   * @arg {Object} options The incident action properties to edit
   * @arg {Date?} [options.dmsDisabledUntil] When DMs will be enabled again (set as `null` to enable now)
   * @arg {Date?} [options.invitesDisabledUntil] When invites will be enabled again (set as `null` to enable now)
   * @returns {Promise<{dmsDisabledUntil: Date?, dmSpamDetectedAt: Date?, invitesDisabledUntil: Date?, raidDetectedAt: Date?}>} An object containing guild incident data
   */
  editIncidentActions(options) {
    return this._client.editGuildIncidentActions.call(this._client, this.id, options);
  }
