...
class Guild extends Base {
    constructor(data, client) {
        ...
        this.channels = new Collection(GuildChannel, client._collectionLimit);
        this.members = new Collection(Member, client._collectionLimit);
        this.memberCount = data.member_count || data.members.length;
        this.roles = new Collection(Role, client._collectionLimit);

        for(var channel of data.channels) {
            channel = this.channels.add(channel, this);
            if(!client.options.restMode) {
        ...
        if(data.voice_states) {
            if(!client.bot) {
                ...
                for(var voiceState of data.voice_states) {
                    ...
                    if(!client.options.restMode && client.options.seedVoiceConnections && voiceState.id === client.user.id && !client.voiceConnections.get(this.id)) {
