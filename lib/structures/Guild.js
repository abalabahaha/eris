...
const ThreadChannel = require('eris/lib/structures/ThreadChannel');
const StickerItem = require('./StickerItem');
const SlashCommandGuild = require('../slashcommand/SlashCommandGuild');
...
class Guild extends Base {
  constructor(data, client) {
    ...
    this.slashCommand = new SlashCommandGuild(client, this)
    ...
    if (data.stickers !== undefined) {
      this.stickers = new Collection(StickerItem);
      for (const sticker of data.stickers) {
        this.stickers.add(new StickerItem(sticker));
      }
    }
    ...
    if (data.threads !== undefined) {
      for (const channelData of data.threads) {
        channelData.guild_id = this.id;
        const channel = Channel.from(channelData, client);
        channel.guild = this;
        this.channels.add(channel, client);
  ...
  update(data) {
    ...
    if (data.threads !== undefined) {
      for (const channelData of data.threads) {
        channelData.guild_id = this.id;
        const channel = Channel.from(channelData, this._client);
        channel.guild = this;
        this.channels.add(channel, this._client);
        this._client.channelGuildMap[channel.id] = this.id;
      }
    }
  }
  ...
  createSticker(name, description, tags, file) {
    return this._client.createGuildSticker.call(this._client, name, description, tags, file);
  }
