...
class SequentialBucket {
    ...
    */
    ...
    check(override) {
        ...
        if(this.reset && this.reset < Date.now() - this.latency) {
            this.reset = this.resetInterval ? Date.now() - this.latency + this.resetInterval : 0;
        ...
        if(this.remaining <= 0) {
            return setTimeout(() => {
                this.check(true);
            }, Math.max(0, this.reset || 0 - Date.now()) + this.latency);
        }
        --this.remaining;
        this._queue.shift()(() => {
