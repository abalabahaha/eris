...
class RequestHandler {
    constructor(client, forceQueueing) {
        ...
        this.readyQueue = [];
        this.invalidHeaderRegex = /[^\t\x20-\x7e\x80-\xff]/;
        if(forceQueueing) {
    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return this._request(method, url, auth, body, file, route, short).then(
            (res) => res,
            (rej) => {
                if(!(rej instanceof DiscordHTTPError || rej instanceof DiscordRESTError)) {
    ...
    _request(method, url, auth, body, file, _route, short) {
        const route = _route || this.routefy(url, method);

        const _stackHolder = {}; // Preserve async stack
        Error.captureStackTrace(_stackHolder);

        if(body && this.invalidHeaderRegex.test(body.reason)) {
            throw new TypeError("Invalid character in audit log reason");
        }
