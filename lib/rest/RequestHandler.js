        ...
        return new Promise((resolve, reject) => {
                    ...
                    resp.once("end", () => {
                        ...
                        if(resp.statusCode >= 300) {
                            if(resp.statusCode === 429) {
                                this._client.emit("warn", `${resp.headers["x-ratelimit-global"] ? "Global" : "Unexpected"} 429 (╯°□°）╯︵ ┻━┻: ${response}\n${body && body.content} ${Date.now()} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[route].remaining}/${this.ratelimits[route].limit} left | Reset ${this.ratelimits[route].reset} (${this.ratelimits[route].reset - Date.now()}ms left)`);
                                if(resp.headers["retry-after"]) {
                                    setTimeout(() => {
            ...
            if(this.globalBlock && auth) {
                this.readyQueue.push(() => {
                    ...
                    ++waitFor;
                    this.ratelimits[route].queue(done, short);
                });
            } else {
                if(!this.ratelimits[route]) {
                    this.ratelimits[route] = new SequentialBucket(1, this.latencyRef);
