...
class RequestHandler {
    constructor(client, forceQueueing) {
        ...
        if(forceQueueing || (this._client.bot === undefined && forceQueueing === undefined)) {
        ...
        var actualCall = (cbs) => {
            ...
            req.once("response", (resp) => {
                ...
                resp.once("end", () => {
                    ...
                    this.ratelimits[url].remaining = resp.headers["x-ratelimit-remaining"] === undefined ? 1 : +resp.headers["x-ratelimit-remaining"] || 0;
                    ...
                    this._client.emit("debug", `${resp.statusCode} ${latency}ms ${this.ratelimits[url].remaining}/${this.ratelimits[url].limit} ${this.ratelimits[url].reset} (${this.ratelimits[url].reset - Date.now()})`);
        ...
        if(this.globalBlock && auth) {
            this.readyQueue.push(() => {
                if(!this.ratelimits[url]) {
                    this.ratelimits[url] = new SequentialBucket(1);
                }
                ++waitFor;
