...
class ChannelUpdate extends EventCursor {
  ...
  onEvent(packet) {
    ...
    if (channel instanceof GroupChannel) {
      ...
      this.shardController.emit('warn', `Unexpected CHANNEL_UPDATE for channel ${packet.d.id} with type ${oldType}`);
    }
    const oldType = channel.type;
    if (oldType === packet.d.type) {
      channel.update(packet.d);
    } else {
      this.shardController.emit('debug', `Channel ${packet.d.id} changed from type ${oldType} to ${packet.d.type}`);
      const newChannel = Channel.from(packet.d, this.client);
      if (packet.d.guild_id) {
        const guild = this.client.guilds.get(packet.d.guild_id);
        if (!guild) {
          this.shardController.emit('debug', `Received CHANNEL_UPDATE for channel in missing guild ${packet.d.guild_id}`);
          return;
        }
        ...
        this.shardController.emit('warn', new Error('Unhandled CHANNEL_UPDATE type: ' + JSON.stringify(packet, null, 2)));
        return;
      }
      channel = newChannel;
    }
     ...
     */
    this.shardController.emit('channelUpdate', channel, oldChannel);
    return;
  }
}
