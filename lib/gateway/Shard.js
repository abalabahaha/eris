    ...
    */
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                const nonce = packet.d.nonce || packet.d.guild_id;

                if(this.requestMembersPromise.hasOwnProperty(nonce)) {
                    this.requestMembersPromise[nonce].members.push(...members);
                    if(packet.d.chunk_count <= (packet.d.chunk_index + 1)) {
                        clearTimeout(this.requestMembersPromise[nonce].timeout);
                        this.requestMembersPromise[nonce].res(this.requestMembersPromise[nonce].members);
                        delete this.requestMembersPromise[nonce];
    ...
    getGuildMembers(guildID, timeout) {
        ...
        if(this.client.options.intents) {
            ...
            return this.requestGuildMembers([guildID], timeout).then((m) => m.length);
        } else {
    ...
    requestGuildMembers(guildID, options) {
        const opts = {
            ...
            nonce: options && options.query && Math.random().toString(36),
            presences: options && options.presences
        };
        if(options && Array.isArray(options.query)) {
            opts.user_ids = options.query;
        } else {
            opts.query = (options && options.query) || "";
        }

        if(!opts.query && !(this.client.options.intents & Constants.Intents.guildMembers)) {
            throw new Error("Cannot request all members without guildMembers intent");
        }
        ...
        const promiseKey = opts.nonce || guildID;
        return new Promise((res) => this.requestMembersPromise[promiseKey] = {
            res: res,
            received: 0,
            members: [],
            timeout: setTimeout(() => {
                res(this.requestMembersPromise[promiseKey].members);
                delete this.requestMembersPromise[promiseKey];
