  ...
  wsEvent(packet) {
    switch (packet.t) {
      ...
      case "GUILD_SOUNDBOARD_SOUND_DELETE": {
        const guild = this.client.guilds.get(packet.d.guild_id);
        if (!guild) {
          this.emit("guildSoundboardSoundDelete", new SoundboardSound(packet.d, this.client));
          break;
        }
        this.emit("guildSoundboardSoundDelete", guild.soundboardSounds.remove(packet.d)); // FIXME No `id` in structure
        /*
      ...
      case "SOUNDBOARD_SOUNDS": {
        // TODO Replace what's in cache. Add logic to and for requestGuildSoundboardSounds
        break;
      }
      default: {
        /**
