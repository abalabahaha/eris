    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                let event = guild.events.get(packet.d.id);
                ...
                if(event) {
                    oldEvent = {
                        ...
                        entiyType: event.entityType
                    };
                }
                event = guild.events.update(packet.d, this.client);

                /**
                * Fired when a scheduled guild event is updated
                * @event Client#guildScheduledEventUpdate
                * @prop {GuildEvent} event The updated event
                * @prop {Object?} oldEvent The old guild event data, or null if the event wasn't cached.
                * @prop {String} oldEvent.channelID The channel ID of the event
                * @prop {String} oldEvent.name The name of the event
                * @prop {Number} oldEvent.privacyLevel The privacy level of the event
                * @prop {Date} oldEvent.scheduledStartTime The time the event will start
                * @prop {String} oldEvent.description The description of the event
                * @prop {Number} oldEvent.entityType The [scheduled entity type](https://discord.com/developers/docs/resources/guild_event/guild-event-object-guild-scheduled-event-entity-types) of the event
                */
                this.emit("guildScheduledEventUpdate", event, oldEvent);
                break;
            }
