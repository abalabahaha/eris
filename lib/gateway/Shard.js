    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "INVITE_CREATE": {
                ...
                */
                this.emit("inviteCreate", guild.invites.add(new Invite({
                    ...packet.d,
                    guild,
                    channel
                }, this.client)));
            ...
            case "INVITE_DELETE": {
                ...
                const invite = guild.invites.get(packet.d.code);
                if(!invite) {
                    this.emit("debug", `Missing invite ${packet.d.code} in INVITE_DELETE`);
                    break;
                }
                guild.invites.remove(invite);
                /**
