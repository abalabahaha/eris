    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                const event = new GuildEvent(packet.d, this.client);

                const guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("debug", `Missing guild ${packet.d.guild_id} in GUILD_SCHEDULED_EVENT_CREATE`);
                    break;
                }

                guild.events.add(packet.d, this.client);
                this.client.guildEventMap[packet.d.id] = packet.d.guild_id;

                /**
                * Fired when a scheduled guild event is created
                * @event Client#guildScheduledEventCreate
                * @prop {GuildEvent} event The event
                */
                this.emit("guildScheduledEventCreate", event);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                const cachedEvent = guild.events.get(packet.d.id);

                if(!cachedEvent) {
                    this.emit("guildScheduledEventUpdate", guild.events.add(packet.d, this.client), null);
                    this.client.guildEventMap[packet.d.id] = packet.d.guild_id;
                    break;
                }
                ...
                const event = {
                    channelID: packet.d.channel_id,
                    name: packet.d.name,
                    privacyLevel: packet.d.privacy_level,
                    scheduledStartTime: Date.parse(packet.d.scheduled_start_time),
                    description: packet.d.description,
                    entityType: packet.d.entity_type
                };
                cachedEvent.update(packet.d);

                /**
                * Fired when a scheduled guild event is updated
                * @event Client#guildScheduledEventUpdate
                * @prop {Object} event The updated event
                * @prop {String} event.channelID The channel ID of the event
                * @prop {String} event.name The name of the event
                * @prop {Number} event.privacyLevel The privacy level of the event
                * @prop {Number} event.scheduledStartTime The time to schedule the event
                * @prop {String} event.description The description of the event
                * @prop {Number} event.entityType The scheduled entity type of the event
                * @prop {?GuildEvent} oldEvent The old guild event. This will be null if the event was uncached
                */
                this.emit("guildScheduledEventUpdate", event, cachedEvent);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_DELETE": {
                delete this.client.guildEventMap[packet.d.id];
                const guild = this.client.guilds.get(packet.d.guild_id);

                if(!guild) {
                    this.emit("debug", `Missing guild ${packet.d.guild_id} in GUILD_SCHEDULED_EVENT_DELETE`);
                    break;
                }
                const event = guild.events.remove(packet.d);
                if(!event) {
                    break;
                }
                /**
                * Fired when a scheduled guild event is deleted
                * @event Client#guildScheduledEventDelete
                * @prop {GuildEvent} event The event that was deleted.
                */
                this.emit("guildScheduledEventDelete", event);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_USER_CREATE": {
                const guild = this.client.guildEventMap[packet.d.guild_scheduled_event_id];
                const user = this.client.users.get(packet.d.user_id) || packet.d.user_id;

                if(!guild) {
                    this.emit("guildScheduledEventUserCreate", packet.d.guild_scheduled_event_id, user);
                    break;
                }
                ...
                * @event Client#guildScheduledEventUserCreate
                * @prop {GuildEvent | string} event The guild event that the user subscribed to. This will be the guild event ID if the guild was uncached
                * @prop {User | string} user The user that subscribed to the Guild Event. This will be the user ID if the user was uncached
                */
                this.emit("guildScheduledEventUserCreate", event, user);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_USER_DELETE": {
                const guild = this.client.guildEventMap[packet.d.guild_scheduled_event_id];
                const user = this.client.users.get(packet.d.user_id) || packet.d.user_id;

                if(!guild) {
                    this.emit("guildScheduledEventUserDelete", packet.d.guild_scheduled_event_id, user);
                    break;
                }
                ...
                * @event Client#guildScheduledEventUserDelete
                * @prop {GuildEvent | string} event The guild event that the user unsubscribed to. This will be the guild event ID if the guild was uncached
                * @prop {User | string} user The user that unsubscribed to the Guild Event. This will be the user ID if the user was uncached
                */
                this.emit("guildScheduledEventUserDelete", event, user);
                break;
            }
