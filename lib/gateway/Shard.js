    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_MEMBER_UPDATE": {
                ...
                // Detect which roles has been removed from the member,
                // and then, if there is, remove the member id from the correspondent <Role>.members
                const removeMemberFromTheseRoles = oldMember.roles.filter((roleId) => !member.roles.includes(roleId));

                removeMemberFromTheseRoles.forEach((roleId) => {
            ...
            case "GUILD_MEMBER_REMOVE": {
                ...
                member.roles.forEach((role_id) =>{
                    if(guild.roles.has(role_id)) {
                        guild.roles.get(role_id).role.members.filter((member_id) => member_id !== packet.d.user.id);
                    }
                });
            ...
            case "GUILD_CREATE": {
                if(!packet.d.unavailable) {
                    ...
                    packet.d.members.forEach((member) => {
                        member.roles.forEach((role_id) => {
                            if(guild.roles.has(role_id)) {
                                guild.roles.get(role_id).members.push(member.id);
                            }
                        });
                    });
