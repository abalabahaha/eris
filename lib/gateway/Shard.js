...
class Shard extends EventEmitter {
    ...
    */
                ...
                if(this.getAllUsersCount.hasOwnProperty(guild.id)) {
                    if(this.getAllUsersCount[guild.id] <= 1) {
                        ...
                        if(this.requestMembersPromise.hasOwnProperty(guild.id)) {
                            ...
                            this.requestMembersPromise[guild.id].res([...this.requestMembersPromise[guild.id].members, ...members]);
                        ...
                        if(this.requestMembersPromise.hasOwnProperty(guild.id)) {
                            this.requestMembersPromise[guild.id].members.push(...members);
                        }
                    }
                } else if(this.requestMembersPromise.hasOwnProperty(packet.d.nonce)) {
                    clearTimeout(this.requestMembersPromise[packet.d.nonce].timeout);
                    this.requestMembersPromise[packet.d.nonce].res([...this.requestMembersPromise[packet.d.nonce].members, ...members]);
    ...
    requestGuildMembers(guildID, timeout, query, limit) {
        ...
        return new Promise((res) => this.requestMembersPromise[options.nonce || guildID] = {
            res: res,
            received: 0,
            members: [],
            timeout: setTimeout(() => {
                res(this.requestMembersPromise[options.nonce].members);
                delete this.requestMembersPromise[options.nonce];
