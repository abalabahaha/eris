  ...
  editStatus(status, game) {
    ...
    this.client.guilds.forEach((guild) => {
      if (guild.shard.id === this.id) {
        if (guild.members.get(this.client.user.id) !== undefined) {
  ...
  wsEvent(packet) {
    switch (packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
      ...
      case 'INTERACTION_UPDATE': {
        this.emit('interactionUpdate', new Interaction(packet.d, this.client, this.guilds.get(packet.d.guild_id),
          this.guilds.get(packet.d.guild_id).members.get(packet.d.member.user.id),
          this.guilds.get(packet.d.guild_id).channels.get(packet.d.channel_id)));
        break;
      }
      case 'INTERACTION_DELETE': {
        this.emit('interactionDelete', new Interaction(packet.d, this.client, this.guilds.get(packet.d.guild_id),
          this.guilds.get(packet.d.guild_id).members.get(packet.d.member.user.id ?? packet.d.member_id),
          this.guilds.get(packet.d.guild_id).channels.get(packet.d.channel_id)));
        break;
      }
      case 'INTERACTION_CREATE': {
        if (packet.d.type === 2) {
          this.emit('slashCommand', new Interaction(packet.d, this.client, this.client.guilds.get(packet.d.guild_id) ?? undefined,
        ...
        this.emit('interactionCreate', new Interaction(packet.d, this.client, this.client.guilds.get(packet.d.guild_id) ?? undefined,
          this.client.guilds.get(packet.d.guild_id).members.get(packet.d.member.user.id ?? packet.d.member_id),
