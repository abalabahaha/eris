...
const ButtonInteracion = require("../structures/Button");
...
class Shard extends EventEmitter {
    ...
    */
    ...
    wsEvent(packet) {
        //if (packet.t == 'INTERACTION_CREATE') console.log(packet)
        switch (packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻

            case "INTERACTION_CREATE": {

                console.log(packet);
                if (!((packet.d?.token) && (packet.d?.id) && (packet.d) && (packet.d.data) && (packet.d.data.component_type == 2)))
                    return;

                const res = new ButtonInteracion(packet.d, this.client);

                /**
                * @event Client#clickButton
                * @prop {ButtonInteracion} res The updated user
                */
                this.emit("clickButton", res)
            ...
            case "PRESENCE_UPDATE": {
                if (packet.d.user.username !== undefined) {
                    ...
                    if (!user || oldUser) {
                        user = this.client.options.cacheUsers ? this.client.users.update(packet.d.user, this.client) : new User(packet.d.user, this.client);
                        /**
                ...
                if ((!member && packet.d.user.username) || oldPresence) {
                    ...
                    member = this.client.options.cacheMembers ? guild.members.update(packet.d.member, guild) : new Member(packet.d.member, guild);
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                if (!member) {
                    // Updates the member cache with this member for future events. [XD]
                    packet.d.member.id = packet.d.user_id;

                    member = this.client.options.cacheMembers ? guild.members.add(packet.d.member, guild) : new Member(packet.d.member, guild);
            ...
            case "MESSAGE_REACTION_ADD": {
                ...
                if (channel) {
                    ...
                    if (channel.guild) {
                        if (packet.d.member) {
                            // Updates the member cache with this member for future events. [JAJAJAJAJA]
                            packet.d.member.id = packet.d.user_id;
                            member = this.client.options.cacheMembers ? channel.guild.members.update(packet.d.member, channel.guild) : new Member(packet.d.member, channel.guild);
                        }
                    }
                }
                if (message) {
            ...
            case "GUILD_MEMBER_ADD": {
                ...
                const member = this.client.options.cacheMembers ? guild.members.add(packet.d, guild) : new Member(packet.d, guild);
                ++guild.memberCount;
                /**
            ...
            case "GUILD_MEMBER_UPDATE": {
                ...
                member = this.client.options.cacheMembers ? guild.members.update(packet.d, guild) : new Member(packet.d, guild);
                /**
            ...
            case "GUILD_BAN_ADD": {
                const user = this.client.options.cacheUsers ? this.client.users.update(packet.d.user, this.client) : new User(packet.d.user, this.client);
                console.log(user);
                /**
                ...
                */
                this.emit("guildBanAdd", this.client.guilds.get(packet.d.guild_id), user);
                break;
            }
            case "GUILD_BAN_REMOVE": {
                ...
                * @prop {User} user The banned user
                */
                this.emit("guildBanRemove", this.client.guilds.get(packet.d.guild_id), user);
                break;
            }
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                const members = packet.d.members.map((member) => {
                    ...
                    return this.client.options.cacheMembers ? guild.members.add(member, guild) : new Member(member, guild);
                });
            ...
            case "GUILD_SYNC": {// (╯°□°）╯︵ ┻━┻ thx Discord devs
                ...
                for (const member of packet.d.members) {
                    member.id = member.user.id;
                    this.client.options.cacheMembers ? guild.members.add(member, guild) : new Member(member, guild);
                }
                ...
                for (const presence of packet.d.presences) {
                    ...
                    const member = this.client.options.cacheMembers ? guild.members.update(presence) : new Member(presence, guild);
                }
            ...
            case "GUILD_EMOJIS_UPDATE": {
                ...
                if (guild) {
                    ...
                    emojis = this.client.options.cacheEmojis ? guild.update(packet.d) : new Guild(packet.d, this.client);
                }
                /**
