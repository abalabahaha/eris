    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_SCHEDULED_EVENT_CREATE": {
                ...
                * @event Client#guildEventCreate
                * @prop {GuildEvent} event The event
                */
                this.emit("guildEventCreate", event);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_UPDATE": {
                ...
                if(!event) {
                    this.emit("guildEventUpdate", guild.threads.add(packet.d, this.client), null);
                    this.client.guildEventMap[packet.d.id] = packet.d.guild_id;
                }
                ...
                const oldEvent = {
                    ...
                    scheduledStartTime: new Date(packet.d.scheduled_start_time),
                    description: packet.d.description,
                    entityType: packet.d.entity_type
                };
                event.update(packet.d);

                /**
                * Fired when a scheduled guild event is updated
                * @event Client#guildEventUpdate
                * @prop {GuildEvent} event The updated event
                * @prop {?Object} oldEvent The old guild event. This will be null if the event was uncached
                ...
                * @prop {Date} oldEvent.scheduledStartTime The time to schedule the event
                * @prop {String} oldEvent.description The description of the event
                * @prop {Number} oldEvent.entityType The scheduled entity type of the event
                */
                this.emit("guildEventUpdate", event, oldEvent);
                break;
            }
            case "GUILD_SCHEDULED_EVENT_DELETE": {
                ...
                * @event Client#guildEventDelete
                * @prop {GuildEvent} event The event that was deleted
                */
                this.emit("guildEventDelete", event);
                break;
            }
