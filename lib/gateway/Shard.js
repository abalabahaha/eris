...
class Shard extends EventEmitter {
    ...
    */
    ...
    wsEvent(packet) {
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                ...
                if(!guild) {
                    this.emit("warn", "Rogue presence update: " + JSON.stringify(packet), this.id);
                    break;
                }
                ...
                if(packet.d.guild_id === undefined) {
                    ...
                    if(packet.d.channel_id === null) {
                        ...
                        if(!flag) {
                            ...
                            if(!flag) {
                                this.emit("error", new Error("VOICE_STATE_UPDATE for user leaving call not found"));
                                break;
                            }
                        }
                    } else {
                        ...
                        if(!channel.call && !channel.lastCall) {
                            this.emit("error", new Error("VOICE_STATE_UPDATE for untracked call"));
                            break;
                        }
            ...
            case "GUILD_MEMBER_ADD": {
                ...
                if(!guild) { // Eventual Consistency™ (╯°□°）╯︵ ┻━┻
                    this.emit("warn", `Missing guild ${packet.d.guild_id} in GUILD_MEMBER_ADD`);
                    break;
                }
                packet.d.id = packet.d.user.id;
                ++guild.memberCount;
            ...
            case "GUILD_MEMBER_UPDATE": {
                var guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("warn", `Missing guild ${packet.d.guild_id} in GUILD_MEMBER_UPDATE`);
                    break;
                }
                var member = guild.members.get(packet.d.id = packet.d.user.id);
                var oldMember = null;
            ...
            case "GUILD_BAN_ADD": {
                ...
                */
                this.emit("guildBanAdd", this.client.guilds.get(packet.d.guild_id), this.client.users.add(packet.d.user, this.client));
                break;
            }
            case "GUILD_BAN_REMOVE": {
                ...
                * @prop {User} user The banned user
                */
                this.emit("guildBanRemove", this.client.guilds.get(packet.d.guild_id), this.client.users.add(packet.d.user, this.client));
                break;
            }
            case "GUILD_ROLE_CREATE": {
                ...
                */
                var guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("warn", `Missing guild ${packet.d.guild_id} in GUILD_ROLE_CREATE`);
                    break;
                }
            ...
            case "GUILD_ROLE_UPDATE": {
                var guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("warn", `Guild ${packet.d.guild_id} undefined in GUILD_ROLE_UPDATE`);
                    break;
                }
            ...
            case "GUILD_ROLE_DELETE": {
                ...
                */
                var guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
                    this.emit("warn", `Missing guild ${packet.d.guild_id} in GUILD_ROLE_DELETE`);
                    break;
                }
            ...
            case "CHANNEL_CREATE": {
                if(packet.d.type === undefined || packet.d.type === 1) {
                    ...
                    var guild = this.client.guilds.get(packet.d.guild_id);
                    if(!guild) {
                        this.emit("warn", `Missing guild ${packet.d.guild_id} in CHANNEL_CREATE`);
                        break;
                    }
                    if(packet.d.type === 0) {
                        var channel = guild.channels.add(new TextChannel(packet.d, guild), guild);
                    } else if(packet.d.type === 2) {
                        var channel = guild.channels.add(new VoiceChannel(packet.d, guild), guild);
                    } else if(packet.d.type === 4) {
                        var channel = guild.channels.add(new CategoryChannel(packet.d, guild), guild);
                    } else {
                    ...
                    if(packet.d.parent_id) {
                        ...
                        if(!parentChannel) {
                            this.emit("warn", `Missing category channel ${packet.d.parent_id} in CHANNEL_CREATE`);
                            break;
                        }
            ...
            case "CHANNEL_UPDATE": {
                ...
                if(packet.d.parent_id) {
                    ...
                    var parentChannel = guild.channels.get(packet.d.parent_id);
                    if(!parentChannel) {
                        this.emit("warn", `Missing category channel ${packet.d.parent_id} in UPDATE`);
                        break;
                    }
                    parentChannel.channels.add(channel);
                }
            ...
            case "CHANNEL_DELETE": {
                if(packet.d.type === 1 || packet.d.type === undefined) {
                    ...
                    var guild = this.client.guilds.get(packet.d.guild_id);
                    if(!guild) {
                        this.emit("warn", `Missing guild ${packet.d.guild_id} in CHANNEL_DELETE`);
                        break;
                    }
            ...
            case "CHANNEL_RECIPIENT_ADD": {
                var channel = this.client.groupChannels.get(packet.d.channel_id);
                    ...
                    * @prop {User} user The user
                    */
                this.emit("channelRecipientAdd", channel, channel.recipients.add(this.client.users.add(packet.d.user, this.client)));
                break;
            }
            ...
            case "PRESENCES_REPLACE": {
                for(var presence of packet.d) {
                    ...
                    if(!guild) {
                        this.emit("warn", "Rogue presences replace: " + JSON.stringify(presence), this.id);
                        continue;
                    }
        ...
        // this.emit("debug", packet.t + ": " + (Date.now() - startTime) + "ms" + debugStr, this.id);
    }
    ...
    initializeWS() {
        ...
        this.ws.onclose = (event) => {
            ...
            var reconnect = "auto";
            if(event.code) {
                this.emit("warn", `${event.code === 1000 ? "Clean" : "Unclean"} WS close: ${event.code}: ${event.reason}`, this.id);
                if(event.code === 4001) {
    ...
    */
    ...
    emit() {
        ...
        super.emit.apply(this.client, arguments);
    }
}
