...
class Shard extends EventEmitter {
    ...
    */
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                if(this.requestMembersPromise.hasOwnProperty(nonce)) {
                    if(packet.d.chunk_count < packet.d.chunk_index) {
                        this.requestMembersPromise[nonce].members.push(...members);
                    } else {
                        clearTimeout(this.requestMembersPromise[nonce].timeout);
                        this.requestMembersPromise[nonce].res([...this.requestMembersPromise[nonce].members, ...members]);
    ...
    requestGuildMembers(guildID, timeout, query, limit) {
        const options = {
            guild_id: guildID,
            limit: limit || 0,
            nonce: query && Math.random().toString(36)
        };
        if(Array.isArray(query)) {
            options.user_ids = query;
        } else {
            options.query = query || "";
        }
        this.sendWS(GatewayOPCodes.GET_GUILD_MEMBERS, options);
        const promiseKey = options.nonce || guildID;
