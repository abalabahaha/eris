        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                if(packet.d.user.username !== undefined) {
                    ...
                    let oldUser = null;
                    if(user && (user.username !== packet.d.user.username || user.discriminator !== packet.d.user.discriminator || user.avatar !== packet.d.user.avatar || user.banner !== packet.d.user.banner || user.accentColor !== packet.d.user.accent_color)) {
                        oldUser = {
                    ...
                    if(!user || oldUser) {
                        ...
                        /**
                        * Fired when a user's username, discriminator, avatar, banner or banner color changes
                        ...
                        * @prop {String?} oldUser.banner The hash of the user's banner, or null if no banner
                        * @prop {Number?} oldUser.accentColor The user's banner color, or null if no banner color
                        */
            ...
            case "THREAD_UPDATE": {
                ...
                if(!channel) {
                    ...
                    this.client.guilds.get(packet.d.guild_id).threads.add(thread, this.client);
                    this.emit("threadUpdate", channel, null);
                    break;
                }
            ...
            case "THREAD_MEMBERS_UPDATE": {
                ...
                */
                this.emit("threadMembersUpdate", channel, removedMembers, addedMembers);
                break;
            }
