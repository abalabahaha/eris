            ...
            case "PRESENCE_UPDATE": {
                if(packet.d.user.username !== undefined) {
                    ...
                    if(!user || oldUser) {
                        ...
                        * @prop {String} oldUser.discriminator The discriminator of the user. If they've migrated to the new username system, this will be "0"
            ...
            case "GUILD_SYNC": {// (╯°□°）╯︵ ┻━┻ thx Discord devs
                ...
                for(const presence of packet.d.presences) {
                    if(!guild.members.get(presence.user.id)) {
                        ...
                        if(userData) {
                            userData = `{username: ${userData.username}, id: ${userData.id + (userData.discriminator === "0" ? "" : `, discriminator: ${userData.discriminator}`)}}`;
                        }
