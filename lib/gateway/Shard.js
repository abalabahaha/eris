        ...
        switch (packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                if (packet.d.user.username !== undefined) {
                    ...
                    if (!user || oldUser) {
                        ...
                        if (!this.client.options.cacheUsers) this.client.users.remove(packet.d.user);
                        /**
                ...
                if ((!member && packet.d.user.username) || oldPresence) {
                    ...
                    if (!this.client.options.cacheMembers && (member.id != this.client.user.id))
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                if (!member) {
                    ...
                    if (!this.client.options.cacheMembers && (member.id != this.client.user.id))
                        guild.members.remove(packet.d.member);
                ...
                if (channel) {
                    ...
                    if (channel.guild) {
                        if (packet.d.member) {
                            ...
                            if (!this.client.options.cacheMembers && (member.id != this.client.user.id))
                                channel.guild.members.remove(packet.d.member);
                        }
                    }
                }
                if (message) {
            ...
            case "GUILD_MEMBER_ADD": {
                ...
                const member = guild.members.add(packet.d, guild);
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                */
                ...
                if (!this.client.options.cacheMembers) {
                    members.map(item => item.id != this.client.user.id ? guild.members.remove(item) : null);
                }
                this.lastHeartbeatAck = true;
            ...
            case "GUILD_SYNC": {// (╯°□°）╯︵ ┻━┻ thx Discord devs
                ...
                if (!this.client.options.cacheMembers)
                    for (const member of Array.from(guild.members).map(item => item[1]).filter(item => item.id != this.client.user.id))
                ...
                for (const presence of packet.d.presences) {
                    ...
                    const member = guild.members.update(presence);
                    if (!this.client.options.cacheMembers && (member.id != this.client.user.id))
                        guild.members.remove(member);
                }
                if (guild.pendingVoiceStates && guild.pendingVoiceStates.length > 0) {
                    for (const voiceState of guild.pendingVoiceStates) {
                        ...
                        if (channel) {
                            ...
                            /*if (!this.client.options.cacheMembers && (voiceState.id != this.client.user.id))
                                guild.members.remove(voiceState);*/
