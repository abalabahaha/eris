  ...
  wsEvent(packet) {
    switch (packet.t) {
      ...
      case "SOUNDBOARD_SOUNDS": {
        const guild = this.client.guilds.get(packet.d.guild_id);
        const sounds = guild ? guild.soundboardSounds : packet.d.soundboard_sounds.map((sound) => new SoundboardSound(sound, this.client)); // TODO Add to cache. WARN No `id` in structure
