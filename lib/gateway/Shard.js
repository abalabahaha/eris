...
* @prop {Object} status Status of the shard
* @prop {String} status.code Status code of the shard(either closed, connected or error)
* @prop {String} [status.message] The error message if the shard status is error
*/
class Shard extends EventEmitter {
    ...
    */
    ...
    wsEvent(packet) {
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                if(!member) {
                    var channel = guild.channels.find((channel) => (channel.type === 2 || channel.type === "voice") && channel.voiceMembers.get(packet.d.id));
                    if(channel) {
    ...
    heartbeat(normal) {
        if(normal && !this.lastHeartbeatAck && this.client.options.gatewayVersion >= 5) {
