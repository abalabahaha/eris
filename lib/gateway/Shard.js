  ...
  wsEvent(packet) {
    switch (packet.t) {
      ...
      case "GUILD_SOUNDBOARD_SOUND_CREATE": {
         ...
         */
        this.emit("guildSoundboardSoundCreate", guild.soundboardSounds.add(packet.d, this.client)); // FIXME No `id` in structure
        break;
      }
      case "GUILD_SOUNDBOARD_SOUND_DELETE": {
         ...
         * @prop {SoundboardSound} sound The deleted soundboard sound. If the soundboard sound isn't cached, this will be an object with `soundID` and `guild` keys. If the guild isn't cached, it will be an object with an `id` key. No other properties are guaranteed
         */
        this.emit("guildSoundboardSoundDelete", (guild && guild.soundboardSounds.remove(packet.d)) || { // FIXME No `id` in structure
        ...
        /*
        const sound = guild.soundboardSounds.get(packet.d.sound_id) || null;
        guild.soundboardSounds.delete(packet.d.sound_id);
        this.emit("guildSoundboardSoundDelete", sound)
        */
        break;
      }
      case "GUILD_SOUNDBOARD_SOUND_UPDATE": {
        ...
        const sound = guild.soundboardSounds.get(packet.d.sound_id);
        let oldSound = null;
        if (sound) {
          oldSound = {
            name: sound.name,
         ...
         */
        this.emit("guildSoundboardSoundUpdate", guild.soundboardSounds.update(packet.d, this.client), oldSound); // FIXME No `id` in structure
        break;
      }
      case "GUILD_SOUNDBOARD_SOUNDS_UPDATE": {
        const guild = this.client.guilds.get(packet.d[0].guild_id);
        if (!guild) {
          this.emit("guildSoundboardSoundsUpdate", packet.d.map((sound) => new SoundboardSound(sound, this.client)), null);
          break;
        }

        const oldSounds = packet.d.map((_sound) => {
          const sound = guild.soundboardSounds.get(_sound.sound_id);
          if (!sound) {
            return null;
          }
          return {
         ...
         */
        this.emit("guildSoundboardSoundsUpdate", packet.d.map((sound) => guild.soundboardSounds.update(sound, this.client)), oldSounds); // FIXME No `id` in structure
        break;
      }
      case "SOUNDBOARD_SOUNDS": {
        const guild = this.client.guilds.get(packet.d.guild_id);
        const sounds = packet.d.soundboard_sounds.map((sound) => guild ? guild.soundboardSounds : new SoundboardSound(sound, this.client)); // FIXME Add properly to cache. No `id` in structure
