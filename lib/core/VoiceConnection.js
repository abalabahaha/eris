...
class VoiceConnection extends EventEmitter {
    ...
    * @arg {Number} [inlineVolume=false] Whether to enable on-the-fly volume changing
    */
        ...
        var send = () => {
            try {
                ...
                if(this.sendPacket(this.createPacket(sequence, timestamp, buffer))) {
                    return setTimeout(send, startTime + sequence * length - Date.now());
                }
    ...
    */
    setVolume(volume) {
        if(!this.volumeTransformer) {
            throw new Error("Could not find volumeTransformer stream. Did you enable inlineVoice?");
        }
