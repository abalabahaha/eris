...
class VoiceConnection extends EventEmitter {
    constructor(guildID, shard) {
        ...
        try {
            ...
            this.opus = new (require("../../deps/opusscript/OpusScript.js"))(48000, 2);
        }
        try {
        ...
        var send = () => {
            try {
                ...
                if(this.paused && this.playing) {
                    pausedTime += 5 * length;
                    this.timestamp += 5 * length;
                    if(this.timestamp >= 4294967295) {
                        this.timestamp -= 4294967295;
                    }
                    return setTimeout(send, 5 * length);
                }
                ...
                if(!buffer && this.playing && (voiceDataTimeout === -1 || waitingForData <= voiceDataTimeout / length)) { // wait for data
                    if(++waitingForData <= 5) {
                        ...
                        pausedTime += 2 * length;
                        this.timestamp += 2 * length;
                        if(this.timestamp >= 4294967295) {
                            this.timestamp -= 4294967295;
                        }
                        return setTimeout(send, 2 * length);
                    }
