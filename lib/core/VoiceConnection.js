...
class VoiceConnection extends EventEmitter {
    constructor(guildID, shard) {
        ...
        this.packetBuffer = Buffer.alloc(12 + 16 + 1276 * 3);
    ...
    */
    ...
    registerReceiveEventHandler() {
        this.udpSocket.on("message", (msg) => {
            var nonce = Buffer.from(msg.buffer, 0, 12);
            ...
            if(!this.receiveStreamPCM.pcm || (data = this.opus.decode(data, 1920))) {
                this.receiveStreamPCM.emit("data", data, this.ssrcUserMap[nonce.readUIntBE(8, 4)]);
            } else {
                this.emit("warn", "Failed to decode received packet");
            }
        });
    }
            ...
            this.ws.on("message", (m) => {
                ...
                switch(packet.op) {
                    case OPCodes.HELLO: {
                        ...
                        DNS.lookup(this.endpoint, (err, address) => { // RIP DNS
                            ...
                            */
                            ...
                            var udpMessage = Buffer.alloc(70);
                    ...
                    case OPCodes.HEARTBEAT: {
                        this.emit("debug", "Heartbeat received: " + (Date.now() - packet.d) + "ms");
                        break;
                    }
        ...
        var onReadable = () => {
            this.playRaw(stream, (source) => {
                ...
                if (buffer.length !== this.pcmSize) {
                    var scratchBuffer = Buffer.alloc(this.pcmSize);
