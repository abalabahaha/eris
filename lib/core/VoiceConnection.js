...
class VoiceConnection extends EventEmitter {
    ...
    */
    playStream(stream, options) {
        ...
        pipe.once('unpipe', onError);
        pipe.once('end', onError);

        stream.once("error", onError);

        encoder.once("close", onError);
        encoder.stdin.once("error", onError);
        encoder.stdout.once("error", onError);

        var stdout = encoder.stdout;

        if(options.inlineVolume) {
            this.volumeTransformer.attach(encoder.stdout);
            stdout = this.volumeTransformer;

            this.volumeTransformer.once("error", onError);
            this.volumeTransformer.once("unattach", onError);
        }

        this.playRawStream(stdout, options);
    }
    ...
    */
        ...
        var send = () => {
            try {
                ...
                if(this.paused && this.playing) {
                    pausedTime += 10 * length;
                    this.timestamp += 10 * length;
                ...
                if(!buffer && this.playing && (voiceDataTimeout === -1 || waitingForData <= voiceDataTimeout / length)) { // wait for data
                    ++waitingForData;
    ...
    createPacket(buffer) {
        var packetBuffer = new Buffer(buffer.length + 12 + 16);
        packetBuffer.fill(0);
        packetBuffer[0] = 0x80;
        packetBuffer[1] = 0x78;
