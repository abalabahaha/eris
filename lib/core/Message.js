...
* @prop {?Guild} guild The guild the message is in
...
class Message {
    ...
    update(data, client) {
        ...
        this.mentions = (this.content.match(/<@\!?[0-9]+>/g) || []).map((mention) => {
            ...
            var user = this.guild && this.guild.members.get(id) || client.users.get(id);
            if(user) {
        ...
        if(client.options.moreMentions) { // TODO mix with nick regex
            if(this.guild) {
                var roles = this.guild.members.get(client.user.id).roles;
                this.roleMentions = (this.content.match(/<@&[0-9]+>/g) || []).map((mention) => {
                    var id = mention.substring(3, mention.length - 1);
                    var role = this.guild.roles.get(id);
                    if(role) {
                        this.cleanContent = this.cleanContent.replace(mention, `@${role.name}`);
                    }
                    return id;
                });
                roles = roles.filter((role) => ~this.roleMentions.indexOf(role)); // TODO check permissions
                if(roles.length > 0) {
