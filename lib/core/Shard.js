...
class Shard extends EventEmitter {
    ...
    */
    ...
    identify() {
        ...
        if(this.client.options.userAccount && this.client.options.gatewayVersion >= 5) {
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "CALL_CREATE": {
                ...
                if(channel.call) {
                    ...
                    channel.call = new Call(packet.d);
                    var incrementedID = "";
                    var overflow = true;
            ...
            case "CHANNEL_RECIPIENT_ADD": { // TODO implement
                this.client.groupChannels.get(packet.d.channel_id).recipients.add(this.client.users.add(packet.d.user));
                break;
            }
            case "CHANNEL_RECIPIENT_REMOVE": { // TODO implement
                this.client.groupChannels.get(packet.d.channel_id).recipients.remove(packet.d.user);
                break;
            }
            ...
            case "READY": {
                ...
                packet.d.private_channels.forEach((channel) => {
                    if(channel.type === 1) {
                        ...
                        this.client.groupChannels.add(channel, this.client).sync();
                    } else {
            ...
            case "VOICE_SERVER_UPDATE": {
                var connection = this.client.voiceConnections.get(packet.d.guild_id || "Group");
