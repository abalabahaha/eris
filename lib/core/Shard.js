    ...
    wsEvent(packet) {
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "READY": {
                ...
                if(packet.d.user.bot === this.client.options.userAccount) {
                    throw new Error(`Client option userAccount is set to ${this.client.options.userAccount}, but the bot user is ${packet.d.user.bot ? "a" : "not a"} user account`);
                }
                ...
                if(packet.d.relationships) {
                    packet.d.relationships.forEach((relationship) => {
                        var a = this.client.relationships.add(relationship, this.client);
                    });
                }
