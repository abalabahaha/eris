        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "VOICE_STATE_UPDATE": { // (╯°□°）╯︵ ┻━┻
                ...
                if(!member) {
                    ...
                    this.client.emit("warn", new Error("VOICE STATE UPDATE MEMBER NULL " + JSON.stringify(packet)), this.id);
                    break;
                }
                var oldState = {
                    mute: member.mute,
            ...
            case "CHANNEL_PINS_UPDATE": {
                this.getChannel(packet.d.channel_id).lastPinTimestamp = Date.parse(packet.d.timestamp);
                break;
            }
