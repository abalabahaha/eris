    ...
    async process(args, msg) {
        ...
        if(this.cooldown !== 0 && !this.cooldownCheck(msg)) {
            ...
            if(this.cooldownMessage && (!this.cooldownReturns || this.cooldownAmounts[msg.author.id] <= this.cooldownReturns)) {
                reply = typeof this.cooldownMessage === "function" ? await Promise.resolve(this.cooldownMessage(msg)) : this.cooldownMessage;
                if(reply) {
        ...
        if(!await this.permissionCheck(msg)) {
            ...
            reply = typeof this.permissionMessage === "function" ? await Promise.resolve(this.permissionMessage(msg)) : this.permissionMessage;
            if(reply) {
                msg.channel.createMessage(reply);
            }
            return;
        ...
        if(args.length === 0) {
            ...
            if(this.argsRequired) {
                ...
                reply = typeof this.invalidUsageMessage === "function" ? await Promise.resolve(this.invalidUsageMessage(msg)) : this.invalidUsageMessage;
                if(reply) {
