    ...
    async process(args, msg) {
        ...
        if(args.length === 0) {
            ...
            if(this.argsRequired) {
                ...
                reply = typeof this.invalidUsageMessage === "function" ? this.invalidUsageMessage(msg) : this.invalidUsageMessage.replace(/%prefix%/g, msg.prefix).replace(/%label%/g, this.fullLabel);
                if(reply) {
                    msg.channel.createMessage(reply);
                }
                return;
