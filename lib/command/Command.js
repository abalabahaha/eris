...
class Command {
    ...
    */
    ...
    process(args, msg, client) {
        ...
        if (this.deleteCommand && msg.channel.guild && msg.channel.permissionsOf(client.user.id).has('manageMessages')) {
            let originalExec = this.execute;
            this.execute = (msg, args) => {
                client.deleteMessage(msg.channel.id, msg.id);
                return originalExec(msg, args);
            };
        }
        if(args.length === 0) {
        ...
        if((subcommand = this.subcommands[label]) !== undefined || ((subcommand = this.subcommands[label.toLowerCase()]) !== undefined && subcommand.caseInsensitive)) {
            return subcommand.process(args.slice(1), msg, client);
        } else {
            if(this.cooldown !== 0 && !this.cooldownCheck(msg.author.id)) {
                if(this.cooldownMessage) {
                    msg.channel.createMessage(this.cooldownMessage);
