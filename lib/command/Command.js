...
class Command {
    ...
    * @arg {Number} [cooldown] The cooldown between command usage in milliseconds
    * @arg {String} [options.cooldownMessage] A message to show when command is on cooldown
    */
    ...
    permissionCheck(msg) {
        ...
        if(msg.member && msg.member.roles.length > 0) {
    ...
    process(args, msg) {
        ...
        if(args.length === 0) {
            if(this.cooldown !== 0 && !this.cooldownCheck(msg.author.id)) {
                if(this.cooldownMessage) msg.channel.createMessage(this.cooldownMessage);
                return;
            }
