    ...
    onMessageCreate(msg) {
        ...
        if((!this.commandOptions.ignoreSelf || msg.author.id !== this.user.id) && (!this.commandOptions.ignoreBots || !msg.author.bot) && (msg.prefix = this.checkPrefix(msg))) {
            ...
            if((command = this.commands[label]) !== undefined || ((command = this.commands[label.toLowerCase()]) !== undefined && command.caseInsensitive)) {
                ...
                args.forEach(arg => {
                    if (command.subcommands[arg] && command.subcommands[arg].deleteCommand)
                        if (msg.channel.guild && msg.channel.permissionsOf(this.user.id).has('manageMessages')) {
                            this.deleteMessage(msg.channel.id, msg.id);
                        }
                });
                if(command.deleteCommand || command && msg.channel.guild && msg.channel.permissionsOf(this.user.id).has('manageMessages')) {
                    this.deleteMessage(msg.channel.id, msg.id);
                }
            }
        }
