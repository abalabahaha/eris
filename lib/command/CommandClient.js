...
class CommandClient extends Client {
    ...
    */
    constructor(token, options, commandOptions) {
        ...
        this.on("messageReactionAdd", this.onMessageReactionAdd);
                    ...
                    if(resp != null) {
                        this.createMessage(msg.channel.id, resp).then((newMsg) => {
                            if(command.reactionButtons) {
                                command.buttons.forEach(button => newMsg.addReaction(button));
                                this.activeMessages[newMsg.id] = {
                                    label: command.caseInsensitive ? label.toLowerCase() : label,
                                    invoker: msg.author.id,
                                    timeout: setTimeout(() => {
                                        this.unwatchMessage(newMsg.id);
                                    }, command.reactionButtonTimeout * 1000)
                                };
                            }
                        });
                    }
    ...
    onMessageReactionAdd(msg, emoji, userID) {
        ...
        var activeMessage = this.activeMessages[msg.id], command, action;
        if(activeMessage !== undefined && (command = this.commands[activeMessage.label]) !== undefined) {
            if((action = (command.reactionButtons.find(button => button.emoji === emoji))) === undefined) {
                return;
            }
            if(activeMessage.invoker !== userID) {
                return;
            }

            switch(action.type) {
                case "cancel": {
                    this.unwatchMessage(msg.id);
                    break;
                }
                case "edit": {
                    Promise.resolve(action.execute()).then((resp) => {
                        if(resp != null) {
    ...
    * @arg {Array<Object>} [options.reactionButtons] Buttons mapped by reaction to add to the message, which preform an action on the message when pressed.
    * @arg {String} [options.reactionButtons.emoji] Emoji which should activate this button. Custom emojis should be in format `emojiName:emojiID`
    * @arg {String} [options.reactionButtons.type] Type of action when a reaction button is clicked. Either "edit" or "cancel".
    * @arg {Function | String | Array<Function | String>} [options.reactionButtons.response] If type is "edit", the content in which the message will be edited with. See createMessage docs for object structure.
    * @arg {Number} [options.reactionButtonTimeout] Time (in seconds) to wait before invalidating a command's reaction buttons.
    * @returns {Command}
    */
