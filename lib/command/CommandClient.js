...
class CommandClient extends Client {
    ...
    constructor(token, options, commandOptions) {
        ...
        this.on("messageCreate", (msg) => {
            ...
            if((!this.commandOptions.ignoreSelf || msg.author.id !== this.user.id) && (!this.commandOptions.ignoreBots || !msg.author.bot) && (msg.prefix = this.checkPrefix(msg))) {
                ...
                label = (this.commandAliases[label.toLowerCase()] && this.commands[this.commandAliases[label.toLowerCase()]].caseInsensitive ? this.commandAliases[label.toLowerCase()] : this.commandAliases[label] || label);
                var command;
    ...
    */
    registerCommandAlias(alias, label) {
        ...
        if(!this.commands[label] && !(this.commands[label.toLowerCase()] && (caseInsensitiveLabel = this.commands[label.toLowerCase()].caseInsensitive))) {
            throw new Error("No command registered for " + label);
        }
        alias = caseInsensitiveLabel === true ? alias.toLowerCase() : alias;
        label = caseInsensitiveLabel === true ? label.toLowerCase() : label;
