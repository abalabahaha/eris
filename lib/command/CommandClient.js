...
class CommandClient extends Client {
    ...
    */
    ...
    onMessageReactionAdd(msg, emoji, userID) {
        ...
        if(activeMessage !== undefined && (command = this.commands[activeMessage.label]) !== undefined) {
            if((action = (command.reactionButtons[emoji])) === undefined) {
                return;
            }
            ...
            switch(action.type) {
                ...
                case "edit": {
                    this.editMessage(msg.channel.id, msg.id, action.content);
                    break;
                }
            }
        }
    ...
    * @arg {Object} [options.reactionButtons] Buttons mapped by reaction to add to the message, which preform an action on the message when pressed.
    * @arg {String} [options.reactionButtons[btn].type] Type of action when a reaction button is clicked. Either "edit" or "cancel".
    * @arg {Object} [options.reactionButtons[btn].content] If type is "edit", the content in which the message will be edited with. See createMessage docs for object structure.
