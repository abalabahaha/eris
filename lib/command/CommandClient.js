...
class CommandClient extends Client {
    ...
    */
                    ...
                    if(resp != null) {
                        this.createMessage(msg.channel.id, resp).then((newMsg) => {
                            if(command.reactionButtons) {
                                newMsg.chainReaction(command.buttons);
    ...
    onMessageReactionAdd(msg, emoji, userID) {
        ...
        if(activeMessage !== undefined && (command = this.commands[activeMessage.label]) !== undefined) {
            ...
            switch(action.type) {
                case "cancel": {
                    delete this.activeMessages[msg.id];
                    break;
                }
                case "edit": {
    ...
    * @arg {Function | String | Array<Function | String>} [options.reactionButtons.content] If type is "edit", the content in which the message will be edited with. See createMessage docs for object structure.
