...
class VoiceConnection extends EventEmitter {
    constructor(guildID, userID, sessionID, shard) {
        ...
        this.piper = new Piper(this.converterCommand, this.opus, this.pcmSize, this.frameSize, this.frameDuration);
    ...
    */
    connect(channelID) {
        ...
        this.voiceServerUpdateCallback = (data) => {
            ...
            this.ws.on("message", (m) => {
                ...
                switch(packet.op) {
                    ...
                    case OPCodes.HEARTBEAT: {
                        ...
                        * @prop {Number} The current server -> client latency in milliseconds
                        */
            ...
            }, this.shard ? this.shard.client.options.connectionTimeout : 10000);
        };
    }
    ...
    * @arg {String} [options.encoding] The format of the resource. If null, FFmpeg will attempt to guess and play the format. Available options: "dca", "ogg", "pcm", null
    ...
    */
    play(source, options) {
        ...
        if(!this.piper.encode(source, options)) {
            this.emit("error", new Error("Unable to resolve source"));
            return;
        }
    ...
    _play(options) {
        ...
        var send = () => {
            ...
            if(this.paused && this.playing) {
                pausedTime += 4 * this.frameDuration;
                this.timestamp += 4 * this.frameSize;
                if(this.timestamp >= 4294967295) {
                    this.timestamp -= 4294967295;
                }
                return setTimeout(send, 4 * this.frameDuration);
            }
            ...
            } else if(options.voiceDataTimeout === -1 || waitingForData < options.voiceDataTimeout / this.frameDuration) { // wait for data
                if(++waitingForData <= 5) {
    ...
    */
    ...
    pickCommand() {
        ...
        if(tenative) {
            this.converterCommand = tenative;
            return new Error(tenative + " does not have libopus support. Non-opus playback may be laggy");
        }
