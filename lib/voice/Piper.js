...
class Piper extends EventEmitter {
    ...
    stop(e, source) {
        ...
        if(this.streams.find((stream) => stream._readableState && stream._readableState.pipesCount)) {
            return;
        }
    ...
    addDataPacket(packet) {
        ...
        if(this._dataPackets.push(packet) < 30) {
            process.nextTick(() => this._endStream.transformCB());
        }
    }
    ...
    getDataPacket() {
        if(this._dataPackets.length < 15) {
