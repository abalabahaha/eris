...
class Piper extends EventEmitter {
    ...
    encode(source, options) {
        ...
        if(options.format === "dca") {
            ...
            if(options.inlineVolume) {
                ...
                this.streams.push(source = source.pipe(new FFmpegOggTransformer(this.converterCommand, this.frameDuration, options.format)).once("error", (e) => this.stop(e)));
                this.streams.push(source.pipe(new OggOpusTransformer().once("error", (e) => this.stop(e))));
            }
        } else {
        ...
        this.streams[this.streams.length - 1].once("finish", () => this.stop(null, source));
    ...
    addDataPacket(packet) {
        ...
        if(this.dataPackets.push(packet) > 50) {
            ...
            this.throttleTimeout = setTimeout(() => this.streams[this.streams.length - 1].resume(), 20 * 20);
        }
    }
