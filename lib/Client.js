...
const GuildChannel = require("./core/GuildChannel");
...
class Client extends EventEmitter {
    ...
    */
    constructor(token, options) {
        ...
        if(typeof Endpoints.BASE_URL === "function") {
            Endpoints.BASE_URL = Endpoints.BASE_URL(this.options.gatewayVersion);
        }
        ...
        if(this.options.buckets) {
            this.buckets["bot:global"] = new Bucket(50, 1000, this.options.sequencerWait);
    ...
    */
    getGateway() {
        ...
        return this.callAPI("GET", Endpoints.GATEWAY).then((data) => {
            if(data.url) {
    ...
    */
    getChannelInvites(channelID) {
        return this.callAPI("GET", Endpoints.CHANNEL_INVITES(channelID), true).then((invites) => invites.map((invite) => new Invite(invite)));
    }
    ...
    */
    getInvite(inviteID) {
        return this.callAPI("GET", Endpoints.INVITE(inviteID), true).then((invite) => {
            if(this.channelGuildMap[invite.channel.id] && this.getChannel(invite.channel.id).permissionsOf(this.user.id).json.manageChannels) { // TODO verify this is the right permission
    ...
    */
    addRelationship(userID, block) {
        return this.callAPI("PUT", Endpoints.ME_RELATIONSHIP(userID), true, {
    ...
    */
    removeRelationship(userID) {
        return this.callAPI("DELETE", Endpoints.ME_RELATIONSHIP(userID), true);
    }
    ...
    */
    addGroupRecipient(groupID, userID) {
        return this.callAPI("PUT", Endpoints.GROUP_RECIPIENT(groupID, userID), true);
    }
    ...
    */
    removeGroupRecipient(groupID, userID) {
        return this.callAPI("DELETE", Endpoints.GROUP_RECIPIENT(groupID, userID), true);
    }

    bucketFromRequest(method, url, body) {
        ...
        if(match) {
            if(method === "DELETE" && this.getChannel(match[1])) {
                ...
                if(method === "POST" || method === "PATCH") {
                    if(this.privateChannels.get(match[1])) {
                        return "bot:msg:dm:" + match[1];
                    } else if(this.channelGuildMap[match[1]]) {
                        return "bot:msg:guild:" + this.channelGuildMap[match[1]];
                    }
                }
            } else {
                return ["msg"];
            ...
            if(url === "/users/@me" && this.user && body && body.username !== this.user.username) {
                return "username";
    ...
    */
    callAPI(method, url, auth, body, file) {
        ...
        var bucket = this.bucketFromRequest(method, url, body);
        var attempts = 1;
        var actualCall = () => {
            var headers = {
                    ...
                    if(resp.statusCode >= 300) {
                        if(resp.statusCode === 429) {
                            if(this.options.buckets) {
                                ...
                                setTimeout(actualCall, this.buckets[bucket] = response.retry_after);
                                this.buckets[bucket] += Date.now();
                                return;
                            }
        ...
        if(this.options.buckets) {
            ...
            if(this.bot) {
                ++waitFor;
                this.buckets["bot:global"].queue(done);
            }
            if(bucket) {
                ++waitFor;
            ...
            if(this.buckets[bucket]) {
                if(this.buckets[bucket] <= Date.now) {
                    this.buckets[bucket] = null;
                } else {
                    setTimeout(actualCall, this.buckets[bucket]);
                }
            } else {
                actualCall();
            }
