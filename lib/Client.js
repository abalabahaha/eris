...
class Client extends EventEmitter {
    ...
    */
    executeWebhook(webhookID, token, options) {
        ...
        if(options.allowedMentions || this.options.allowedMentions) {
            const allowed = options.allowedMentions ? options.allowedMentions : this.options.allowedMentions;
            options.allowed_mentions = {
                parse: []
            };
            if(allowed.everyone) {
                options.allowed_mentions.parse.push("everyone");
            }
            if(allowed.roles === true) {
                options.allowed_mentions.parse.push("roles");
            }else if(Array.isArray(allowed.roles)) {
                options.allowed_mentions.roles = allowed.roles;
            }
            if(allowed.users === true) {
                options.allowed_mentions.parse.push("users");
            }else if(Array.isArray(allowed.users)) {
                options.allowed_mentions.users = allowed.users;
            }
        }
    ...
    */
    createMessage(channelID, content, file) {
        if(content !== undefined) {
            ...
            if(content.allowedMentions || this.options.allowedMentions) {
                const allowed = content.allowedMentions ? content.allowedMentions : this.options.allowedMentions;
                content.allowed_mentions = {
                    parse: []
                };
                if(allowed.everyone) {
                    content.allowed_mentions.parse.push("everyone");
                }
                if(allowed.roles === true) {
                    content.allowed_mentions.parse.push("roles");
                }else if(Array.isArray(allowed.roles)) {
                    content.allowed_mentions.roles = allowed.roles;
                }
                if(allowed.users === true) {
                    content.allowed_mentions.parse.push("users");
                }else if(Array.isArray(allowed.users)) {
                    content.allowed_mentions.users = allowed.users;
                }
            }
        } else if(!file) {
            return Promise.reject(new Error("No content, file, or embed"));
