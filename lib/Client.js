...
const Shard = require("./connections/Shard");
const User = require("./structures/User");
const VoiceConnection = require("./connections/VoiceConnection");
...
class Client extends EventEmitter {
    ...
    * @arg {Number} [options.guildCreateTimeout=2000] Timeout when waiting for guilds before the ready event
    ...
    */
    getGateway() {
        ...
        return this.requestHandler.request("GET", Endpoints.GATEWAY).then((data) => {
            if(data.url) {
    ...
    * @arg {String} [options.icon] The icon of the channel as a base64 data URI (group channels only)
    ...
    */
    editChannelPosition(guildID, channelID, position) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_CHANNELS(guildID), true, channels.map((channel, index) => ({
            id: channel.id,
            position: index + min
        })));
    }
    ...
    */
    getChannelInvites(channelID) {
        return this.requestHandler.request("GET", Endpoints.CHANNEL_INVITES(channelID), true).then((invites) => invites.map((invite) => new Invite(invite)));
    }
    ...
    */
    createRole(guildID) {
        return this.requestHandler.request("POST", Endpoints.GUILD_ROLES(guildID), true).then((role) => new Role(role));
    }
    ...
    */
    editRole(guildID, roleID, options) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_ROLE(guildID, roleID), true, {
            ...
            permissions: options.permissions !== undefined ? options.permissions : role.permissions,
            color: options.color,
            hoist: options.hoist
    ...
    */
    acceptInvite(inviteID) {
        return this.requestHandler.request("POST", Endpoints.INVITE(inviteID), true).then((invite) => new Invite(invite));
    }
    ...
    */
    getSelf() {
        return this.requestHandler.request("GET", Endpoints.ME, true).then((data) => new ExtendedUser(data));
    }
    ...
    */
    editSelf(options) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.ME, true, {
    ...
    */
    getDMChannel(userID) {
        ...
        return this.requestHandler.request("POST", Endpoints.ME_CHANNELS, true, {
            recipients: [userID],
            type: 1
    ...
    */
    getGuildInvites(guildID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_INVITES(guildID), true).then((invites) => invites.map((invite) => new Invite(invite)));
    }
    ...
    * @arg {String} [icon] The guild icon as a base64 data URI
    ...
    * @arg {String} [options.icon] The guild icon as a base64 data URI
    ...
    * @arg {String} [options.splash] The guild splash image as a base64 data URI (VIP only)
    * @returns {Promise<Guild>}
    */
    ...
    */
    getGuildBans(guildID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_BANS(guildID), true).then((bans) => bans.map((ban) => new User(ban.user)));
    }
    ...
    */
    editNickname(guildID, nick) {
        return this.requestHandler.request("PATCH", Endpoints.GUILD_ME_NICK(guildID), true, {
            nick
        });
    }
    ...
    */
    leaveGuild(guildID) {
        return this.requestHandler.request("DELETE", Endpoints.ME_GUILD(guildID), true);
    }
    ...
    */
    addRelationship(userID, block) {
        return this.requestHandler.request("PUT", Endpoints.ME_RELATIONSHIP(userID), true, {
    ...
    */
    removeRelationship(userID) {
        return this.requestHandler.request("DELETE", Endpoints.ME_RELATIONSHIP(userID), true);
    }
    ...
    */
    addGroupRecipient(groupID, userID) {
        return this.requestHandler.request("PUT", Endpoints.GROUP_RECIPIENT(groupID, userID), true);
    }
    ...
    */
    removeGroupRecipient(groupID, userID) {
        return this.requestHandler.request("DELETE", Endpoints.GROUP_RECIPIENT(groupID, userID), true);
    }
}
