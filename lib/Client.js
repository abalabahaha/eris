...
class Client extends EventEmitter {
    ...
    * @arg {Object} [options.disabledEvents] If disabledEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. <a href="reference.html#ws-event-names">A full list of the WS event names can be found on the docs reference page</a>
    ...
    * @arg {Boolean} [options.moreMentions=true] Whether to activate Message.roleMentions & Message.channelMentions or not
    ...
    * @arg {Number} [options.voiceDataTimeout=2000] How long to wait for voice data before destroying a voice stream
    ...
    */
    joinVoiceChannel(channelID) {
        ...
        if(!channel.permissionsOf(this.user.id).json.voiceConnect) {
            return Promise.reject(new Error("Insufficient permission to connect to voice channel"));
        }
        var shard = this.shards.get(this.guildShardMap[this.channelGuildMap[channelID]]);
        var connection = shard.voiceConnections.get(this.channelGuildMap[channelID]);
        if(connection) {
            ...
            connection = shard.voiceConnections.add(new VoiceConnection(this.channelGuildMap[channelID], shard));
    ...
    */
    leaveVoiceChannel(channelID) {
        ...
        if(connection) {
            ...
            shard.voiceConnections.remove(connection);
        }
    }
    ...
    */
    getChannel(channelID) {
        return this.channelGuildMap[channelID] ? this.guilds.get(this.channelGuildMap[channelID]).channels.get(channelID) : this.privateChannels.get(channelID);
    }
    ...
    */
    editChannel(channelID, options) {
        ...
        return this.callAPI("PATCH", Endpoints.CHANNEL(channelID), true, {
            ...
            position: options.position || channel.position,
    ...
    */
    getInvite(inviteID) {
        return this.callAPI("GET", Endpoints.INVITE(inviteID), true).then((invite) => new Invite(invite)); // TODO check permissions and get extended invite metadata if possible
    }
        ...
        var actualCall = () => {
            ...
            req.on("response", (resp) => {
                ...
                resp.once("end", () => {
                    ...
                    //console.log(resp.statusCode + " " + response.toString());
