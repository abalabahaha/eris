...
module.exports = class Client extends EventEmitter {
    ...
    */
    ...
    /**
     * Gets a shard
     * @param {number} index The number to get
     * @returns {Shard}
     */
    getShard(index) {
        if (this.ws.shards.size >= index)
            throw new SyntaxError(`You only have ${this.ws.shards.size} shards.`);

        return this.ws.shards.get(index);
    }
    ...
    * @param {String | Boolean} [reconnect] false means destroy everything, true means you want to reconnect in the future, "auto" will autoreconnect
    */
    disconnect(options) {
        this.ready = false;
    ...
    * @param {Object} [shared] Whether the VoiceConnection will be part of a SharedStream or not
    * @param {Object} [opusOnly] Skip opus encoder initialization. You should not enable this unless you know what you are doing
    ...
    */
    joinVoiceChannel(channelID, options = {}) {
        ...
        this.ws.shards.get(this.guildShardMap[this.channelGuildMap[channelID]] || 0).sendWS(Constants.GatewayOPCodes.VOICE_STATE_UPDATE, {
    ...
    */
    ...
    closeVoiceConnection(guildID) {
        this.ws.shards.get(this.guildShardMap[guildID] || 0).sendWS(Constants.GatewayOPCodes.VOICE_STATE_UPDATE, {
    ...
    * @param {Object} [game] Sets the bot's active game, null to clear
    * @param {String} game.name Sets the name of the bot's active game
    * @param {Number} [game.type] The type of game. 0 is playing, 1 is streaming (Twitch only), 2 is listening, 3 is watching
    * @param {String} [game.url] Sets the url of the shard's active game
    */
    ...
    * @param {String} [type=0] The type of the channel, either 0 (text), 2 (voice), or 4 (category)
    * @param {String} [reason] The reason to be displayed in audit logs
    * @param {String} [parentID] ID of the parent category for a channel
    ...
    * @param {String} [name] The name of the channel
    * @param {String} [icon] The icon of the channel as a base64 data URI (group channels only). Note: base64 strings alone are not base64 data URI strings
    * @param {String} [ownerID] The ID of the channel owner (group channels only)
    * @param {String} [topic] The topic of the channel (guild text channels only)
    * @param {Boolean} [nsfw] The nsfw status of the channel (guild channels only)
    * @param {Number} [bitrate] The bitrate of the channel (guild voice channels only)
    * @param {Number} [userLimit] The channel user limit (guild voice channels only)
    * @param {String?} [parentID] The ID of the parent channel category for this channel (guild text/voice channels only)
    * @param {String} [reason] The reason to be displayed in audit logs
    ...
    * @param {Number} [maxAge] How long the invite should last in seconds
    * @param {Number} [maxUses] How many uses the invite should last for
    * @param {Boolean} [temporary] Whether the invite is temporary or not
    * @param {Boolean} [unique] Whether the invite is unique or not
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise<Invite>}
    */
    ...
    * @param {String} [avatar] The new default avatar as a base64 data URI. Note: base64 strings alone are not base64 data URI strings
    * @param {String} [token] The token of the webhook, used instead of the Bot Authorization token
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise<Object>} Resolves with a webhook object
    */
    ...
    * @param {Object[]} [embeds] An array of Discord embeds
    * @param {String} [username] A custom username, defaults to webhook default username if not specified
    * @param {String} [avatarURL] A URL for a custom avatar, defaults to webhook default avatar if not specified
    * @param {Boolean} [tts=false] Whether the message should be a TTS message or not
    * @param {Boolean} [wait=false] Whether to wait for the server to confirm the message create or not
    * @param {Boolean} [disableEveryone] Whether to filter @everyone/@here or not (overrides default)
    * @returns {Promise}
    */
    ...
    * @param {String} [before] Get entries before this entry ID
    * @param {Number} [actionType] Filter entries by action type
    ...
    * @param {Array} [roles] An array containing authorized role IDs
    * @param {String} [reason] The reason to be displayed in audit logs
    ...
    * @param {Number} [permissions] The role permissions number
    * @param {Number} [color] The hex color of the role, in number form (ex: 0x3d15b3 or 4040115)
    * @param {Boolean} [hoist] Whether to hoist the role in the user list or not
    * @param {Boolean} [mentionable] Whether the role is mentionable or not
    * @param {String} [reason] The reason to be displayed in audit logs
    ...
    * @param {String} [name] The name of the role
    * @param {Number} [permissions] The role permissions number
    * @param {Number} [color] The hex color of the role, in number form (ex: 0x3da5b3 or 4040115)
    * @param {Boolean} [hoist] Whether to hoist the role in the user list or not
    * @param {Boolean} [mentionable] Whether the role is mentionable or not
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise<Role>}
    ...
    * @param {String} guildID The ID of the guild
    * @param {Number} days The number of days of inactivity to prune for
    ...
    * @param {Boolean} [withCounts] Whether to fetch additional invite info or not (approximate member counts, approximate presences, channel counts, etc.)
    * @returns {Promise<Invite>}
    */
    ...
    * @param {String} [avatar] The new avatar as a base64 data URI. Note: base64 strings alone are not base64 data URI strings
    * @returns {Promise<ExtendedUser>}
    */
    editSelf(options) {
    ...
    * @param {String[]} userIDs The IDs of the other users
    * @returns {Promise<PrivateChannel>}
    */
    ...
    * @param {Object} options The properties to edit
    * @param {String} [expireBehavior] What to do when a user's subscription runs out
    * @param {String} [expireGracePeriod] How long before the integration's role is removed from an unsubscribed user
    * @param {String} [enableEmoticons] Whether to enable integration emoticons or not
    * @returns {Promise}
    */
    ...
    * @param {String} guildID The ID of the guild
    * @param {String} userID The ID of the user
    * @param {Number} [deleteMessageDays=0] Number of days to delete messages for, between 0-7 inclusive
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise}
    */
    ...
    * @param {String} [icon] The guild icon as a base64 data URI. Note: base64 strings alone are not base64 data URI strings
    ...
    * @param {String} guildID The ID of the guild
    * @param {Object} options The properties to edit
    * @param {String} [name] The ID of the guild
    * @param {String} [region] The region of the guild
    * @param {String} [icon] The guild icon as a base64 data URI. Note: base64 strings alone are not base64 data URI strings
    * @param {Number} [verificationLevel] The guild verification level
    * @param {Number} [defaultNotifications] The default notification settings for the guild. 0 is "All Messages", 1 is "Only @mentions".
    * @param {String} [afkChannelID] The ID of the AFK voice channel
    * @param {Number} [afkTimeout] The AFK timeout in seconds
    * @param {String} [ownerID] The ID of the user to transfer server ownership to (bot user must be owner)
    * @param {String} [splash] The guild splash image as a base64 data URI (VIP only). Note: base64 strings alone are not base64 data URI strings
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise<Guild>}
    */
    ...
    * @param {Object} options The properties to edit
    * @param {String[]} [roles] The array of role IDs the member should have
    * @param {String} [nick] Set the member's server nickname, "" to remove
    * @param {Boolean} [mute] Server mute the member
    * @param {Boolean} [deaf] Server deafen the member
    * @param {String} [channelID] The ID of the voice channel to move the member to (must be in voice)
    * @param {String} [reason] The reason to be displayed in audit logs
    * @returns {Promise}
    */
    ...
    * @param {String} [appID="@me"] The client ID of the application to get data for. "@me" refers to the logged in user's own application
    ...
    * @param {Boolean} [block=false] If true, block the user. Otherwise, add the user as a friend
    * @returns {Promise}
    */
    ...
    * @param {String} platform The connection platform (e.g. "twitch", "reddit")
    * @param {String} id The connection ID
    * @param {Object} data The connection data
    * @param {Boolean} [data.friendSync] Whether to sync friends from the connection or not
    * @param {Number} [data.visibility] The visibility level for the connection. 0 = hidden, 1 = shown on profile
    ...
    * @param {Boolean} [data.convertEmoticons] Whether to convert emoticons or not (e.g.:D => 😄)
    * @param {Boolean} [data.detectPlatformAccounts] Whether to automatically detect accounts from other platforms or not (Blizzard, Skype, etc.)
    * @param {Boolean} [data.developerMode] Whether to enable developer mode or not
    * @param {Boolean} [data.enableTTSCommand] Whether to respect usage of the TTS command or not
    * @param {Object} [data.friendSourceFlags] An object representing allowed friend request sources
    * @param {Boolean} [data.friendSourceFlags.all] Whether to allow friends requests from anywhere or not
    * @param {Boolean} [data.friendSourceFlags.mutualFriends] Whether to allow friend requests from people with mutual friends or not
    * @param {Boolean} [data.friendSourceFlags.mutualGuilds] Whether to allow friend requests from people in mutual guilds or not
    * @param {Array<String>} [data.guildPositions] An ordered array of guild IDs representing the guild list order in the Discord client
    * @param {Boolean} [data.inlineAttachmentMedia] Whether to show attachment previews or not
    * @param {Boolean} [data.inlineEmbedMedia] Whether to show embed images or not
    * @param {String} [data.locale] The locale to use for the Discord UI
    * @param {Boolean} [data.messageDisplayCompact] Whether to use compact mode or not
    * @param {Boolean} [data.renderEmbeds] Whether to show embeds or not
    * @param {Boolean} [data.renderReactions] Whether to show reactions or not
    * @param {Array<String>} [data.restrictedGuilds] An array of guild IDs where direct messages from guild members are disallowed
    * @param {Boolean} [data.showCurrentGame] Whether to set the user's status to the current game or not
    * @param {String} [data.status] The status of the user, either "invisible", "dnd", "away", or "online"
    * @param {String} [data.theme] The theme to use for the Discord UI, either "dark" or "light"
    * @returns {Promise<Object>} The user's settings data.
    */
    ...
    * @param {Boolean} [regenerate] Whether to regenerate the MFA backup codes or not
    ...
    * @param {String} secret The TOTP secret used to generate the auth code
    * @param {String} code The timed auth code for the current user
    ...
    * @param {String} token The Stripe credit card token
    * @param {String} plan The plan to purchase, either "premium_month" or "premium_year"
    * @returns {Promise}
    */
    ...
    * @param {Number} [limit=100] The max number of guilds to get (1 to 1000)
    * @param {String} [before] The lowest guild ID of the next page
    * @param {String} [after] The highest guild ID of the previous page
    * @returns {Promise<Guild[]>}
    */
    ...
    * @param {String} guildID The ID of the guild
    * @param {Number} [limit=1] The max number of members to get (1 to 1000)
    * @param {String} [after] The highest user ID of the previous page
    ...
    * @param {String} [query.sortBy="timestamp"] What to sort by, either "timestamp" or "relevance"
    * @param {String} [query.sortOrder="desc"] What order to sort by, either "asc" or "desc"
    * @param {String} [query.content] Filter results by a content string
    * @param {String} [query.authorID] Filter results by an author ID
    * @param {String} [query.minID] The minimum message ID to return results for
    * @param {String} [query.maxID] The maximum message ID to return results for
    * @param {Number} [query.limit=25] How many messages to return, 1 <= limit <= 25
    * @param {Number} [query.offset=0] The query index of the first message to be returned, 0 <= offset <= 5000
    * @param {Number} [query.contextSize=2] How many context messages around each result to return.
    * For example, if you searched for `6` and contextSize was 2, `[4, 5, 6, 7, 8]` would be returned
    * @param {String} [query.has] Only return messages with an "attachment", "embed", or "link"
    * @param {String} [query.embedProviders] Filter results by embed provider
    * @param {String} [query.embedTypes] Filter results by embed type
    * @param {String} [query.attachmentExtensions] Filter results by attachment extension
    * @param {String} [query.attachmentFilename] Filter results by attachment filename
    ...
    * @param {String} [query.attachmentExtensions] Filter results by attachment extension
    * @param {String} [query.attachmentFilename] Filter results by attachment filename
    * @param {String[]} [query.channelIDs] Filter results by channel ID
    * @returns {Promise<Object>} A search result object. The object will have a `totalResults` key and `results` key.
    * Each entry in the result array is an array of Message objects.
    * In each array, the message where `Message.hit === true` is the matched message, while the other messages are context messages.
    * Sample return: ```
 ...
 * @prop {Boolean} [autoreconnect=true] Have Eris autoreconnect when connection is lost
 * @prop {Boolean} [compress=false] Whether to request WebSocket data to be compressed or not
 * @prop {Number} [connectionTimeout=30000] How long in milliseconds to wait for the connection to handshake with the server
 * @prop {{ [x: string]: any; }} [disableEvents] If disableEvents[eventName] is true, the WS event will not be processed. This can cause significant performance increase on large bots. [A full list of the WS event names can be found on the docs reference page](/Eris/docs/reference#ws-event-names)
 * @prop {Boolean} [disableEveryone=true] When true, filter out @everyone/@here by default in createMessage/editMessage
 * @prop {Number} [firstShardID=0] The ID of the first shard to run for this client
 * @prop {Boolean} [getAllUsers=false] Get all the users in every guild. Ready time will be severely delayed
 * @prop {Number} [guildCreateTimeout=2000] How long in milliseconds to wait for a GUILD_CREATE before "ready" is fired. Increase this value if you notice missing guilds
 * @prop {Number} [largeThreshold=250] The maximum number of offline users per guild during initial guild data transmission
 * @prop {Number} [latencyThreshold=30000] The average request latency at which Eris will start emitting latency errors
 * @prop {Number} [lastShardID=options.maxShards - 1] The ID of the last shard to run for this client
 * @prop {Number|String} [maxShards=1] The total number of shards you want to run. If "auto" Eris will use Discord's recommended shard count.
 * @prop {Number} [messageLimit=100] The maximum size of a channel message cache
 * @prop {Boolean} [opusOnly=false] Whether to suppress the node-opus not found error or not
 * @prop {Number} [ratelimiterOffset=0] A number of milliseconds to offset the ratelimit timing calculations by
 * @prop {Number} [requestTimeout=15000] A number of milliseconds before requests are considered timed out
 * @prop {Boolean} [restMode=false] Whether to enable getting objects over REST. This should only be enabled if you are not connecting to the gateway. Bot tokens must be prefixed manually in REST mode
 * @prop {Boolean} [seedVoiceConnections=false] Whether to populate bot.voiceConnections with existing connections the bot account has during startup. Note that this will disconnect connections from other bot sessions
 * @prop {String} [defaultImageFormat="jpg"] The default format to provide user avatars, guild icons, and group icons in. Can be "jpg", "png", "gif", or "webp"
 * @prop {Number} [defaultImageSize=128] The default size to return user avatars, guild icons, and group icons as. Can be 128, 256, 512, 1024, or 2048.
 * @prop {Object} [ws] An object of WebSocket options to pass to the shard WebSocket constructors
 */