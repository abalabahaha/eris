...
class Client extends EventEmitter {
  ...
  createMessage(channelID, content, file) {
    if (content !== undefined) {
      ...
      if (file) {
        ...
        if(Array.isArray(file)) {
          Array.prototype.push.apply(content.files, file.map((file) => file));
        } else {
          content.files.push({
              name: file.name,
              file: file.file
    ...
    return this.requestHandler.request("POST", Endpoints.CHANNEL_MESSAGES(channelID), true, content, content.files).then((message) => new Message(message, this));
  }
   ...
   * @arg {Array<Object>} [options.files] An array of file objects. Only use when creating a thread inside of a forum/media channel
   * @arg {Buffer} [options.files[].file] A buffer containing file data
   * @arg {String} [options.files[].name] What to name the file
   * @arg {Object | Array<Object>} [file] [DEPRECATED] A file object (or an Array of them). Only use when creating a thread inside of a forum/media channel
   * @arg {Buffer} file.file A buffer containing file data
   * @arg {String} file.name What to name the file
  ...
  createThread(channelID, options, file) {
    ...
    if (file) {
      ...
      if(Array.isArray(file)) {
        Array.prototype.push.apply(options.files, file.map((file) => file));
      } else {
        options.files.push({
            name: file.name,
            file: file.file
    ...
    return this.requestHandler.request("POST", Endpoints.THREAD_WITHOUT_MESSAGE(channelID), true, payload, options.files).then((channel) => Channel.from(channel, this));
  }
