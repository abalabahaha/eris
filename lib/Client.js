...
class Client extends EventEmitter {
    ...
    * @arg {Number|String} [options.maxShards=1] The total number of shards you want to run. If `auto` will check with the bot gateway route
    ...
    */
    connect() {
        ...
        return gatewayPromise.then((data) => {
            ...
            if (this.options.maxShards === "auto" && !data.shards) {
                return Promise.reject(new Error("Invalid response from bot gateway REST call"));
            }
            if (data.shards && this.options.maxShards === "auto") {
