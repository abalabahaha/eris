...
class Client extends EventEmitter {
    ...
    */
    ...
    /**
    * Get the bot's own application
    * @returns {Promise<Object>} The bot's application data. Refer to <a href="https://ptb.discordapp.com/developers/docs/topics/oauth2#get-current-application-information">the official Discord API documentation entry</a> for Object structure
    */
    getOAuthApplication() {
        return this.callAPI("GET", Endpoints.OAUTH2_APPLICATION("@me"), true);
    }

    bucketsFromRequest(method, url, body) {
        var match = url.match(/^\/channels\/([0-9]+)\/messages(\/[0-9]+)?$/);
        if(match) {
            if(method === "DELETE" && this.channelGuildMap[match[1]]) {
