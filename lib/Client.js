...
const safeRequire = (dep, fallbackDep = null) => {
    try {
        return require(dep);
    } catch(err) {
        if(fallbackDep !== null) {
...
const safeRequire = (dep, fallbackDep = null, webRequire = false) => {
    try {
      return require(dep);
    } catch (err) {
      if (fallbackDep !== null) {
...
const Erlpack = safeRequire("erlpack");
const ZlibSync = safeRequire("zlib-sync", "pako", true);
