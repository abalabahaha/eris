...
class Client extends EventEmitter {
    ...
    * @arg {Object} [content.messageReference] An object containing reply data
    * @arg {String} [content.messageReference.channelID] The channel ID of the message which should be replied to
    * @arg {String} [content.messageReference.guildID] The guild ID of the message which should be replied to
    * @arg {String} content.messageReference.messageID The ID of the message should be replied to. The reference message cannot be a system message.
    ...
    createMessage(channelID, content, file) {
        if(content !== undefined) {
            ...
            if(content.messageReference) {
                content.message_reference = this._formatMessageReference(content.messageReference);
            }
        } else if(!file) {
            return Promise.reject(new Error("No content, file, or embed"));
        }
    ...
    */
    ...
    _formatMessageReference(messageReference) {
        if(!messageReference.messageID) {
            throw new Error("Message reference must include a message ID");
        }
        return {
