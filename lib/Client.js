...
class Client extends EventEmitter {
    ...
    */
    editRole(guildID, roleID, options) {
        return this.requestHandler.request("PATCH", Endpoints.GUILD_ROLE(guildID, roleID), true, options).then((role) => new Role(role, guild));
    }
    ...
    */
    editSelf(options) {
        return this.requestHandler.request("PATCH", Endpoints.USER("@me"), true, options).then((data) => new ExtendedUser(data));
    }
            ...
            var del = (_before, _after) => {
                this.getMessages(channelID, 100, _before, _after).catch(reject).then((messages) => {
                    ...
                    for(var message of messages) {
                        ...
                        if(message.timestamp < Date.now() - 1209600) { // 14d * 24h * 60m * 60s
                            done = true;
                            return;
                        }
