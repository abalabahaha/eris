...
class Client extends EventEmitter {
    ...
    * @arg {Array<Object>} [content.attachments] An array of attachment objects
    * @arg {Buffer} content.attachments.file A buffer containing attachment data
    * @arg {String} content.attachments.name What to name the attachment
    ...
    */
    createMessage(channelID, content, file) {
        if(content !== undefined) {
            ...
            if(file) {
                this.emit("warn", "[DEPRECATED] File as a seperate argument is deprecated. Use content.attachments instead");
        ...
        return this.requestHandler.request("POST", Endpoints.CHANNEL_MESSAGES(channelID), true, content, content.attachments).then((message) => new Message(message, this));
    }
    ...
    * @arg {Boolean | Array<String>} [content.allowedMentions.roles] Whether or not to allow all role mentions, or an array of specific role mentions to allow.
    * @arg {Boolean | Array<String>} [content.allowedMentions.users] Whether or not to allow all user mentions, or an array of specific user mentions to allow.
    * @arg {Object | Array<Object>} [content.attachments] An array of attachment objects
    * @arg {Buffer} content.attachments.file A buffer containing attachment data
    * @arg {String} content.attachments.name What to name the attachment
    * @arg {Array<Object>} [content.components] An array of component objects
    * @arg {String} [content.components[].custom_id] The ID of the component (type 2 style 0-4 and type 3 only)
    ...
    */
    editMessage(channelID, messageID, content) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.CHANNEL_MESSAGE(channelID, messageID), true, content, content.attachments).then((message) => new Message(message, this));
    }
    ...
    * @arg {Object | Array<Object>} [options.attachments] An array of attachment objects
    * @arg {Buffer} options.attachments.file A buffer containing attachment data
    * @arg {String} options.attachments.name What to name the attachment
    * @arg {Object | Array<Object>} [options.file] [DEPRECATED] A file object (or an Array of them), use options.attachments instead
    ...
    */
    editWebhookMessage(webhookID, token, messageID, options) {
        ...
        if(options.file) {
            this.emit("warn", "[DEPRECATED] options.file as a is deprecated. Use options.attachments instead");
            if(!options.attachments) {
                options.attachments = options.file;
            }
        }
        return this.requestHandler.request("PATCH", Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID), false, options, options.attachments).then((response) => new Message(response, this));
    }
    ...
    */
    executeWebhook(webhookID, token, options) {
        ...
        }, options.attachments).then((response) => options.wait ? new Message(response, this) : undefined);
    }
