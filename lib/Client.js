...
class Client extends EventEmitter {
    ...
    * @returns {Promise<Object>} Resolves with an Object containing `users` and `audit_log_entires` keys
    */
    getGuildAuditLogs(guildID) {
    ...
    * @arg {String} [reason] The ban reason (displayed in audit logs)
    * @returns {Promise}
    */
    ...
    */
    getGuildBans(guildID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_BANS(guildID), true).then((bans) => bans.map((ban) => { return { user: new User(ban.user, this), reason: ban.reason}}));
    }
    ...
    */
    editGuildMember(guildID, memberID, options) {
        if(options.roles) {
            options.roles = options.roles.filter((roleID, index) => options.roles.indexOf(roleID) === index);
        }
