...
class Client extends EventEmitter {
    ...
    * @arg {Object | Array<Object>} [file] [DEPRECATED] A file object (or an Array of them). Use `attachments` instead
    * @arg {Array<Object>} [attachments] An array of attachment objects
    * @arg {Buffer} attachments.file A buffer containing attachment data
    * @arg {String} attachments.name What to name the attachment
    ...
    */
    createMessage(channelID, content, attachments) {
        ...
        return this.requestHandler.request("POST", Endpoints.CHANNEL_MESSAGES(channelID), true, content, attachments).then((message) => new Message(message, this));
    }
    ...
    * @arg {Boolean | Array<String>} [content.allowedMentions.roles] Whether or not to allow all role mentions, or an array of specific role mentions to allow.
    * @arg {Boolean | Array<String>} [content.allowedMentions.users] Whether or not to allow all user mentions, or an array of specific user mentions to allow.
    * @arg {Array<Object>} [content.attachments] attached files to keep (https://discord.com/developers/docs/resources/channel#attachment-object)
    * @arg {Array<Object>} [content.components] An array of component objects
    * @arg {String} [content.components[].custom_id] The ID of the component (type 2 style 0-4 and type 3 only)
    * @arg {Boolean} [content.components[].disabled] Whether the component is disabled (type 2 and 3 only)
    * @arg {Object} [content.components[].emoji] The emoji to be displayed in the component (type 2)
    ...
    * @returns {Promise<Message>}
    */
    editMessage(channelID, messageID, content, attachments) {
        if(content !== undefined) {
            if(typeof content !== "object" || content === null) {
                content = {
                    content: "" + content
        ...
        return this.requestHandler.request("PATCH", Endpoints.CHANNEL_MESSAGE(channelID, messageID), true, content, attachments).then((message) => new Message(message, this));
    }
    ...
    * @arg {Boolean | Array<String>} [options.allowedMentions.users] Whether or not to allow all user mentions, or an array of specific user mentions to allow.
    * @arg {Boolean} [options.allowedMentions.repliedUser] Whether or not to mention the author of the message being replied to.
    * @arg {Array<Object>} [options.attachments] attached files to keep (https://discord.com/developers/docs/resources/channel#attachment-object)
    ...
    * @arg {Object | Array<Object>} [options.file] [DEPRECATED] A file object (or an Array of them), use `attachments` instead
    * @arg {Array<Object>} [attachments] An array of attachment objects
    * @arg {Buffer} attachments.file A buffer containing attachment data
    * @arg {String} attachments.name What to name the attachment
    * @returns {Promise<Message>}
    */
    editWebhookMessage(webhookID, token, messageID, options, attachments) {
        if(options.allowedMentions) {
            options.allowed_mentions = this._formatAllowedMentions(options.allowedMentions);
        }
        if(options.file) {
            this.emit("warn", "[DEPRECATED] options.file is deprecated. Use attachments instead");
            if(!attachments) {
                attachments = options.file;
            }
        }
        return this.requestHandler.request("PATCH", Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID), false, options, attachments).then((response) => new Message(response, this));
    }
    ...
    * @returns {Promise<Message?>}
    */
    executeWebhook(webhookID, token, options, attachments) {
        if(options.file) {
            this.emit("warn", "[DEPRECATED] options.file is deprecated. Use attachments instead");
            if(!attachments) {
                attachments = options.file;
        ...
        }, attachments).then((response) => options.wait ? new Message(response, this) : undefined);
    }
