...
class Client extends EventEmitter {
    ...
    */
    editRole(guildID, roleID, options) {
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_ROLE(guildID, roleID), true, {
            ...
            permissions: options.permissions !== undefined ? options.permissions : (role.permissions && role.permissions.allow),
    ...
    */
    deleteMessages(channelID, messageIDs) {
        ...
        var oldestAllowedSnowflake = (Date.now() - 1421280000000) * Math.pow(2, 22);
        var invalidMessage = messageIDs.find((messageID) => {return parseInt(messageID) < oldestAllowedSnowflake});
        if(invalidMessage) {
            return Promise.reject(new Error(`Message with ID: ${invalidMessage} is too old (>2 weeks) to bulk delete.`));
        }
