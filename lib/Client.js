...
class Client extends EventEmitter {
    ...
    * @returns {Promise}
    */
    deleteGuildEvent(eventID) {
        return this.requestHandler.request("DELETE", Endpoints.GUILD_EVENT(eventID), true);
    }
    /**
    ...
    * @returns {Promise}
    */
    editGuildEvent(event, eventID) {
        return this.requestHandler.request("POST", Endpoints.GUILD_EVENT(eventID), true, {
            channel_id: event.channelID,
            name: event.name,
            privacy_level: event.privacyLevel,
            scheduled_start_time: event.scheduledStartTime,
    ...
    * @returns {Promise}
    */
    getRESTGuildEvent(eventID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Eris REST mode is not enabled"));
        }

        return this.requestHandler.request("GET", Endpoints.GUILD_EVENT(eventID), true).then((data) => new GuildEvent(data, this));
    }
    /**
