...
class Client extends EventEmitter {
    ...
    */
    constructor(token, options) {
        ...
        this._collectionLimit = this.options.state ? undefined : 0;
        ...
        this.groupChannels = new Collection(GroupChannel, this._collectionLimit);
        this.guilds = new Collection(Guild, this._collectionLimit);
        this.privateChannelMap = {};
        this.privateChannels = new Collection(PrivateChannel, this._collectionLimit);
        this.retryAfters = {};
        this.guildShardMap = {};
        this.sharedStreams = {};
        this.unavailableGuilds = new Collection(UnavailableGuild, this._collectionLimit);
        this.relationships = new Collection(Relationship, this._collectionLimit);
        this.users = new Collection(User, this._collectionLimit);
        this.presence = {
            game: null,
            status: "offline"
        };
    ...
    */
    editRole(guildID, roleID, options) {
        var guild = this.guilds.get(guildID);
        var role = (guild && guild.roles.get(roleID)) || {};
    ...
    */
    getDMChannel(userID) {
        ...
        return this.requestHandler.request("POST", Endpoints.USER_CHANNELS("@me"), true, {
            recipients: [userID],
            type: 1
        }).then((privateChannel) => this.privateChannels.add(privateChannel, this));
    }
    ...
    */
    createMessage(channelID, content, file) {
        ...
        if(file && content.embed){
            return Promise.reject(new Error("A file and embed cannot be sent in the same message."))
        }
    ...
    * @returns {Promise}
    */
    removeMessageReactions(channelID, messageID, reaction) {
        if(reaction === decodeURI(reaction)) {
            reaction = encodeURIComponent(reaction);
        }
    ...
    getRESTChannel(channelID) {
        ...
        return this.requestHandler.request("GET", Endpoints.CHANNEL(channelID), true).then((channel) => {
            if(channel.type === 0 || channel.type === 2) {
                return new GuildChannel(channel, false, this._collectionLimit);
    ...
    */
    getRESTGuildChannels(guildID) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD_CHANNELS(guildID), true).then((channels) => channels.map((channel) => new GuildChannel(channel, null, this._collectionLimit)));
    }
