...
createChannelInvite(channelID, options, reason) {
    return this.requestHandler.request('POST', Endpoints.CHANNEL_INVITES(channelID), true, options).then((invite) => new Invite(invite, this));
  }
/**
   *
   * @param command
...
/**
  ...
  deleteCommandsGuild({ commandID, guildID }) {
    return this.requestHandler.request('DELETE', Endpoints.GET_COMMAND_GUILD(this.user.id, guildID, commandID), true, command).then((commandData) => new CommandBase(commandData));
  }
...
/**
   ...
   * @returns {Promise<MessageInteraction>}
   */
  deleteFollowUpMessage(interaction_id, interaction_token, interaction) {
