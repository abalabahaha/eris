...
class Client extends EventEmitter {
    ...
    *
    * @param {boolean} useRecommended Whether to use the recommended shard counts from the Bot Gateway call
    * @returns {Promise} Resolves when all shards are initialized
    */
    connect(useRecommended) {
        useRecommended = useRecommended || false;
        const gatewayPromise = useRecommended ? this.getBotGateway() : this.getGateway();
        ...
        return gatewayPromise.then((data) => {
            ...
            if (useRecommended && data.shards && this.options.maxShards === 1) {
                this.options.maxShards = data.shards;
            }

            if(this.options.lastShardID === undefined) {
                this.options.lastShardID = useRecommended && data.shards ? data.shards - 1 : this.options.maxShards - 1;
            }
