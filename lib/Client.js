...
class Client extends EventEmitter {
    ...
    * @arg {Array<number>} [event.entityMetadata.speakerIDs] The speakers of the stage channel
    ...
    */
    createGuildEvent(guildID, event) {
        ...
        }).then((data) => new GuildEvent(data, this));
    }
    ...
    * @arg {String} guildID The guild id of where the event belongs to
    * @arg {String} eventID The event id
    * @returns {Promise}
    */
    deleteGuildEvent(guildID, eventID) {
    ...
    */
    ...
    /**
    * Edit a scheduled guild event
    * @arg {String} guildID The guild ID where the event will be edited
    * @arg {String} eventID The id of the scheduled guild event to be edited
    ...
    * @returns {Promise<GuildEvent>}
    */
    editGuildEvent(guildID, eventID, event) {
    ...
    */
    ...
    /**
    * Get a guild scheduled event data via the REST API. REST mode is required to use this endpoint.
    * @arg {String} guildID The guild id to get the event of
    * @arg {String} eventID The id of the event
    * @returns {Promise<GuildEvent>}
    */
    getRESTGuildEvent(guildID, eventID) {
        if(!this.options.restMode) {
            return Promise.reject(new Error("Eris REST mode is not enabled"));
        }

        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENT(guildID, eventID), true).then((data) => new GuildEvent(data, this));
    }
    /**
    ...
    */
    ...
    /**
    * List all scheduled events for the given guild.
    * @arg {Boolean} withUserCount Include number of users subscribed to each event
    * @returns {Promise<Array<GuildEvent>>}
    */
    listGuildEvents(guildID, withUserCount) {
        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENTS(guildID), true, {
            with_user_count: withUserCount
        }).then((events) => events.map((event) => new GuildEvent(event, this)));
    }

    listGuildEventUsers(guildID, eventID) {
        return this.requestHandler.request("GET", Endpoints.GUILD_SCHEDULED_EVENT_USERS(guildID, eventID));
    }
