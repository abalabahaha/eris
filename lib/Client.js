...
class Client extends EventEmitter {
  ...
  /**
   * Edit a guild's incident actions
   * @arg {String} guildID The ID of the guild
   ...
   */
  editGuildIncidentActions(guildID, options) {
    return this.requestHandler.request("PUT", Endpoints.GUILD_INCIDENT_ACTIONS(guildID), true, {
      dms_disabled_until: options.dmsDisabledUntil,
      invites_disabled_until: options.invitesDisabledUntil,
    }).then((actions) => {
      return {
        dmsDisabledUntil: actions.dms_disabled_until && Date.parse(actions.dms_disabled_until),
        dmSpamDetectedAt: actions.dm_spam_detected_at && Date.parse(actions.dm_spam_detected_at),
        invitesDisabledUntil: actions.invites_disabled_until && Date.parse(actions.invites_disabled_until),
        raidDetectedAt: actions.raid_detected_at && Date.parse(actions.raid_detected_at),
      };
    });
  }
  ...
  _formatScheduledEventRecurrence(recur) {
    if (!recur) {
      return recur;
    }
    if (recur.byMonth !== undefined) {
      recur.by_month = recur.byMonth;
      delete recur.byMonth;
    }
    if (recur.byMonthDay !== undefined) {
      recur.by_month_day = recur.byMonthDay;
      delete recur.byMonthDay;
    }
    if (recur.byNWeekday !== undefined) {
      recur.by_n_weekday = recur.byNWeekday;
      delete recur.byNWeekday;
    }
    if (recur.byWeekday !== undefined) {
      recur.by_weekday = recur.byWeekday;
      delete recur.byWeekday;
    }
    if (recur.byYearDay !== undefined) {
      recur.by_year_day = recur.byYearDay;
      delete recur.byYearDay;
    }
    return recur;
  }
