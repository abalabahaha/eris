...
class Client extends EventEmitter {
    ...
    */
    ...
    /**
    * Create a guild emoji (not for bot accounts)
    * @arg {String} guildID The ID of the guild to create the emoji in
    * @arg {String} name The name of emoji
    * @arg {String} image The base 64 encoded string
    * @arg {Array} roles An array containing authorized role IDs
    * @returns {Promise}
    */
    createGuildEmoji(guildID, name, image, roles) {
      return this.requestHandler.request("POST", Constants.Endpoints.GUILD_EMOJIS(guildID), true, {
          name: name,
          image: image,
          roles: roles
      })
    ...
    /**
    * Edit a guild emoji (not for bot accounts)
    * @arg {String} guildID The ID of the guild to create the emoji in
    ...
    * @returns {Promise}
    */
    editGuildEmoji(guildID, emojiID, name, image, roles) {
      return this.requestHandler.request("PATCH", Constants.Endpoints.GUILD_EMOJI(guildID, emojiID), true, {
          name: name,
          image: image,
          roles: roles
      })
    ...
    /**
    * Delete a guild emoji (not for bot accounts)
    * @arg {String} guildID The ID of the guild to create the emoji in
