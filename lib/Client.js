...
class Client extends EventEmitter {
    ...
    * @arg {Object | Array<Object>} [file] [DEPRECATED] A file object (or an Array of them). Use `files` instead
    ...
    * @returns {Promise<Message>}
    */
    editMessage(channelID, messageID, content, files) {
        if(content !== undefined) {
            if(typeof content !== "object" || content === null) {
                content = {
                    content: "" + content
    ...
    * @returns {Promise<Message>}
    */
    editWebhookMessage(webhookID, token, messageID, options, files) {
        if(options.allowedMentions) {
            options.allowed_mentions = this._formatAllowedMentions(options.allowedMentions);
        }
        if(options.file) {
            this.emit("warn", "[DEPRECATED] options.file is deprecated. Use files instead");
            if(!files) {
                files = options.file;
            }
        }
    ...
    * @returns {Promise<Message?>}
    */
    executeWebhook(webhookID, token, options, files) {
        if(options.file) {
            this.emit("warn", "[DEPRECATED] options.file is deprecated. Use files instead");
            if(!files) {
                files = options.file;
