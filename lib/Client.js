...
class Client extends EventEmitter {
    ...
    * @arg {Object | Array<Object>} [file] [DEPRECATED] A file object (or an Array of them). Use `content.file` instead
    ...
    */
    createMessage(channelID, content, file) {
        if(file) {
            this.emit("warn", "[DEPRECATED] File as a seperate argument is deprecated. Use content.file instead");
            content.file = file;
        }
        ...
        return this.requestHandler.request("POST", Endpoints.CHANNEL_MESSAGES(channelID), true, content, content.file).then((message) => new Message(message, this));
    }
